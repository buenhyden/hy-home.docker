-- MLflow tracking store provisioning on mng-pg (feature-owned).
--
...
\if :{?mlflow_db_user}
\else
  \echo 'mlflow provisioning: MLFLOW_DB_USER is not set'
  SELECT 'missing MLFLOW_DB_USER'::int;
\endif
\if :{?mlflow_db_name}
\else
  \echo 'mlflow provisioning: MLFLOW_DB_NAME is not set'
  SELECT 'missing MLFLOW_DB_NAME'::int;
\endif
\if :{?mlflow_db_password}
\else
  \echo 'mlflow provisioning: MLFLOW_DB_PASSWORD is not set'
  SELECT 'missing MLFLOW_DB_PASSWORD'::int;
\endif
...
SELECT pg_advisory_lock(hashtext('hy-home:provision:mlflow'));
...
SELECT NOT EXISTS (
         ...
         WHERE r.rolname = :'mlflow_db_user'
           AND (r.rolsuper OR r.rolname = current_user
                OR pg_catalog.shobj_description(r.oid, 'pg_authid')
                   IS DISTINCT FROM 'hy-home:feature:mlflow')
       ) AS role_ok,
       COALESCE((
         SELECT pg_catalog.pg_get_userbyid(datdba) = :'mlflow_db_user'
         FROM pg_catalog.pg_database WHERE datname = :'mlflow_db_name'
       ), true) AS db_owner_ok
\gset
\if :role_ok
\else
  \echo 'mlflow provisioning: MLFLOW_DB_USER names an administrator or a role this job did not create'
  SELECT 'refusing administrator role'::int;
\endif
\if :db_owner_ok
\else
  \echo 'mlflow provisioning: MLFLOW_DB_NAME exists with a different owner'
  SELECT 'refusing foreign database owner'::int;
\endif

BEGIN;
SELECT format('CREATE ROLE %I', :'mlflow_db_user')
WHERE NOT EXISTS (
  SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = :'mlflow_db_user'
)
\gexec
SELECT format('COMMENT ON ROLE %I IS %L', :'mlflow_db_user', 'hy-home:feature:mlflow')
\gexec
COMMIT;
...
SELECT format('CREATE DATABASE %I OWNER %I', :'mlflow_db_name', :'mlflow_db_user')
WHERE NOT EXISTS (
  SELECT 1 FROM pg_catalog.pg_database WHERE datname = :'mlflow_db_name'
)
\gexec
...
SELECT format('REVOKE CONNECT, TEMPORARY ON DATABASE %I FROM PUBLIC', :'mlflow_db_name')
\gexec

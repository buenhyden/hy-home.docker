...
Generate the repo-local LLM Wiki path index through the canonical Python
generator. A sealed Gate 9 manifest is accepted only with --stdout.

Options:
  --check   Fail when the generated index is stale.
...
exec python3 scripts/knowledge/generate-llm-wiki.py --artifact index "$mode"

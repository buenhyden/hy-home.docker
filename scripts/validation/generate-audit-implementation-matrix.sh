...
def build_output() -> tuple[str, list[str]]:
    ...
    lines.extend(
        [
            ...
            "## Traceability",
            "",
            f"- {link(OVERVIEW, 'implementation overview')} - the audited overview categories",
            f"- {link(AUTOMATION_CANDIDATES, 'automation candidates')} - the audited candidate rows",
            f"- {link(pathlib.Path('scripts/validation/generate-audit-implementation-matrix.sh'), 'generator')} - the script that produces this snapshot",
            "",
            "## Related Documents",
            "",

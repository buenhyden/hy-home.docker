...
| Config Mount Hash Check               | [check-config-mount-hashes.py](./operations/check-config-mount-hashes.py)                 | Read-only post-apply check that every running single-file config bind mount matches its host file by SHA-256 (read through `docker exec`, never `docker cp`); exit 1 on any DIFF |

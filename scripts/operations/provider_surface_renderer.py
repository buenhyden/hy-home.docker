...
def render_function(provider: str, function: FunctionRecord) -> bytes:
    if provider not in {"agents-md", "claude", "gemini"}:
        raise ValueError(f"unsupported provider: {provider}")
    body = _frontmatter_free_body(function.source_text)
    description = (
    ...
    for provider in MIGRATION_PROVIDERS:
        ...
        for path in owned.glob("*/SKILL.md"):
            ...
            if relative in expected_paths:
                continue
            try:
                first_chunk = read_repository_text(root, relative.as_posix())[:512]
            except ContractLoadError as error:
                raise ValueError("managed projection entry is unsafe") from error
            if GENERATED_MARKER in first_chunk:
                stale.add(relative)
    return stale
...
def _unlink_confined(root: pathlib.Path, relative: pathlib.Path) -> None:
    with _open_confined_directory(root, relative.parent, create=False) as parent_fd:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise ValueError("managed stale output is unsafe")
        os.unlink(relative.name, dir_fd=parent_fd)


def _remove_empty_parents(root: pathlib.Path, path: pathlib.Path, boundary: pathlib.Path) -> None:
    current = path.parent
    absolute_boundary = root / boundary
    while current != absolute_boundary.parent and current != root:
        try:
            current.rmdir()
        except OSError:
            return
        if current == absolute_boundary:
            return
...
def write_projection(root: pathlib.Path, provider: str) -> None:
    ...
    for relative in sorted(find_managed_stale_files(root, expected)):
        ...
        _unlink_confined(root, relative)
        _remove_empty_parents(root, target, owned_root)

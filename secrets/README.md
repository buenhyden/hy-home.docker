...
`SEC-001` / `secrets/security/vault_token.txt` is the one exact migration-custody
exception to the active declaration/consumer rule. Active Prometheus neither
declares nor mounts it, but the registry identity and any existing local value
remain preserved until a separately approved Vault migration closeout,
credential revocation and file disposition. This exception does not cover any
other orphan row or path.
...
추가한다. 이 작업은 secret 파일을 생성하지 않는다. `SEC-001` 및 unknown

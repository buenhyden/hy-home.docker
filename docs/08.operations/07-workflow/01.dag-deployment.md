...
Compliance is checked via the Airflow static/runtime checks documented in [Airflow Runbook](../../09.runbooks/07-workflow/airflow.md), including `docker compose exec airflow-webserver airflow dags report`, and monthly audits of the Airflow metadata DB.

...
The repository currently exposes three root instruction files: `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md`. It also relies on `docs/00.agent/*.md` and `docs/*/README.md` as linked context. The current documents are partially refactored but still too thin at the root, too generic in `docs/00.agent/`, and inconsistent across documentation indexes.
...
- **Shared Documentation Contract**: Common guidance lives in `docs/00.agent/` and is linked or imported from the root files using relative paths only.
...
```bash
rg -n "^@" CLAUDE.md
rg -n 'file://|docs/99.templates/(architecture|product|operations)/|\]\(/04.specs/' AGENTS.md CLAUDE.md GEMINI.md .claude docs
test -f docs/05.plans/2026-03-12-agent-instruction-refactor.md
test -f docs/04.specs/agent-instructions/spec.md
test -f docs/00.agent/README.md
test -f docs/05.plans/README.md
```
...
- Shared operational details live in `docs/00.agent/*.md`.
...
- **Component Boundary**: Root agent entrypoints, shared `docs/00.agent/*.md` guidance, and documentation-family README indexes under `docs/`.
- **Key Dependencies**: `docs/README.md`, `docs/99.templates/spec.template.md`, `docs/99.templates/plan.template.md`, `.agent/rules/0000-Agents/`, `.agent/rules/2100-Documentation/`
...
| **[REQ-SPC-ADN-002]** | Shared policy must live in repository-local guidance under `docs/00.agent/`. | High | N/A |
...
- **Migration Plan**: Rewrite the roots in place, rebuild `docs/00.agent/*.md` around repo-specific guidance, repair linked docs indexes, and add a plans index without changing product code
...
- `docs/00.agent/README.md`: Shared guidance index
- `docs/00.agent/core-governance.md`: Repo-specific governance, personas, and lazy-loading policy
- `docs/00.agent/workflow.md`: Repo-specific execution workflow and validation commands
- `docs/README.md`: Canonical docs lazy-loading gateway
- `docs/05.plans/README.md`: Tactical plans index
- `docs/05.plans/2026-03-12-agent-instruction-refactor.md`: Execution plan for this refactor
...
- **Missing shared guide path**: If the required `docs/00.agent/` guide files are missing, root files must not be updated to link or import non-existent targets.

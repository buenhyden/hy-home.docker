...
parent_ids: []
archived_from: docs/04.execution/tasks/2026-07-13-document-contract-canonicalization.md
archived_at: 2026-08-11
archive_reason: "Move baseline completed source to stable typed target docs/98.archive/changes/chg-0095-document-contract-canonicalization/task.md; migrate 3 resolved inbound link(s) with it."
archive_disposition: evidence-preserve
archived_commit: 232effd9a5e00907bdbe30efc6665023fb2d07f4
archived_blob: 34bdcf2b3678709b7f69ec6f0340552bb26bd77d
...
- **Parent Spec**:
  [Spec 129](../../tombstones/03.specs/spec-0129-document-contract-canonicalization.md)
- **Parent Plan**:
...
- **Whole-branch review remediation RED**: focused adversarial execution of
  `RepositoryContractIntegrationTests.test_repository_contracts_enforce_readme_profile_frontmatter_behavior`
  and `CheckerCliTests.test_inventory_exposes_all_semantic_state_columns`
  reproduced both binding findings. Contract mode accepted forbidden non-empty
  and empty root README frontmatter and accepted an optional-profile
  disallowed key; the optional-profile allowed `status` key passed as expected.
  Report mode rendered the status-bearing `stage-index` fixture with the false
  `consumer=not-declared` context. The corrected RED run therefore had three
  failing contract subcases and one failing report assertion.
- **Whole-branch review remediation GREEN**: repository-contract validation now
  parses every tracked README with `_record_from_text()`, preserves exact
  unclassified/ambiguous fail-closed classification, and delegates uniquely
  classified profile semantics to `validate_record()`. A forbidden profile now
  rejects any frontmatter fence, including an empty mapping. Inventory
  exception context resolves the matched profile and its registry-declared
  consumer, rendering the status-bearing fixture as
  `profile=stage-index; consumer=scripts/validation/check-document-metadata.py`.
  The focused two-test command passes, including all four contract subcases;
  the full metadata module passes `103/103`; direct `--mode check-contracts`
  reports `violations=0`; `py_compile` and `git diff --check` pass.
- **Remediation sequencing boundary**: the semantic inventory was deliberately
  not regenerated and the controlled wrapper was not rerun in this first fix
  phase. After the controller records a clean checker/tests/evidence commit,
  Task 6 will regenerate owner-managed evidence and run the complete validation
  bundle; only after that second clean commit will it rerun the exact approved
  wrapper and record final evidence. Lifecycle remains active / Review Pending.
- **Post-fix generated boundary**: controller commit `1c2ac7eb` captured the
  checker, adversarial tests, Task 6 evidence, and progress update. Phase 2
  started from that clean worktree and regenerated the semantic inventory only
  through its owner command. The result remains `899` records and `2,025`
  advisory findings; the diff is exactly `148` README exception-context row
  replacements in the one owner-managed inventory path. No handwritten audit
  payload, matrix, Wiki output, or other generated file changed.
- **Post-fix complete validation**: metadata passes `103/103` and validation
  discovery passes `147/147`. Inventory freshness passes at `899/2,025`;
  explicit-base metadata against `e2954cc3` remains `51/0/0/0`; the criterion
  contract and coverage remain `11/161/161` with normalized
...
- **Graphify**: the whole-branch remediation changed checker code, so
  `graphify update .` ran successfully at `22,996` nodes, `24,046` edges, and
  `1,540` communities. The refresh also indexed the ignored pre-closure brief
  and rewrote both tracked graph snapshots; those advisory-only outputs were
  restored from the existing index so the first fix unit contains only
  checker/tests/evidence. Claims were corroborated against the tracked checker,
  tests, Spec 129, active Plan/Task, Stage 00 governance, and direct validation.
...
- **Whole-branch remediation and fresh re-review**: commits `1c2ac7eb`,
  ...
  the tracked branch.
- **Post-closure review**: the independent review of
  `e2954cc3..7d5fbb56` returned Spec FAIL, Quality CHANGES REQUIRED, Critical
  `0`, Important `2`, Minor `0`, and `READY_FOR_HANDOFF: NO`. I-01 found that
  repository-contract README validation lets `generated_by` metadata replace
  the path-derived README profile with the generated-document profile. I-02
  found that current README evidence and the owner-generated audit matrix still
  report `228` after the tracked releases index raised the corpus to `229`.
- **Lifecycle reopening**: the user explicitly approved the reverse
  `completed -> active` transitions for Spec 129, its Plan, and this Task as
  scoped Stage 04 override evidence. T-DCC-006 is Review Pending until both
...
- **Post-closure protected boundary and self-review**: the controlled all-files
  ...
  evidence, and finds Critical `0`, Important `0`, Minor `0`. This was
  implementer evidence only; the later independent reviews are recorded below.
- **Final post-closure handoff review**: a new independent reviewer inspected
  the exact whole-branch range `e2954cc3..9244cdba` after reclosure and returned
  Spec PASS, Quality APPROVED, Critical `0`, Important `0`, Minor `0`, and
  `READY_FOR_HANDOFF: YES`. It confirmed all 229 README profiles, I-01 through
  I-03 remediation, current versus historical audit counts, owner-generated
  freshness, Attempt 4, lifecycle evidence, clean range hygiene, and protected
  boundaries.
...
- **Current review gate**: the post-closure review of
  ...
  C0/I0/M0, and `READY_FOR_RECLOSURE: YES`. After lifecycle reclosure at
  `9244cdba`, a new final reviewer returned Spec PASS, Quality APPROVED,
  C0/I0/M0, and `READY_FOR_HANDOFF: YES` for the exact whole-branch range.
...
- **Pre-closure review remediation**: the focused two-test command passes with
  four README contract subcases; the final metadata module passes `103/103`;
  direct repository contracts report `violations=0`; `py_compile` and diff
  checks pass.
- **Full Test Commands**: the pre-remediation metadata module passed `102/102`
  and validation discovery passed `146/146`. After remediation, the metadata
  module passes `103/103`, validation discovery passes `147/147`, and
  explicit-base changed/new metadata selects `51` paths with `0` violations.

---
title: "Archive Tombstone: Airbyte Usage Guide"
version: 1.0.0
type: archive/tombstone
layer: archive
status: completed
owner: "@buenhyden"
artifact_id: tomb-GDE-0089
parent_ids: [MIG-0001]
created: 2026-06-02
updated: 2026-08-23
---

# Archive Tombstone: Airbyte Usage Guide

## Retired Path

`docs/05.operations/guides/07-workflow/airbyte.md`

## Replacement

`docs/03.specs/0008-workflow/spec.md`

## Reason

Airbyte has no tracked infra implementation under infra/07-workflow/airbyte and must not remain an active operations guide.

## Recovery Commit

`b9a96ac0995fd665f6b121d69483a5c83300823e`

## Traceability

- [Archive index](../../README.md)

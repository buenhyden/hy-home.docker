...
사용자 요청이 `docs/05.plans` 작성 권한을 명시적으로 부여했다. Live audit 기준 `scripts/`에는 root shell script 22개와 `scripts/lib/hardening-lib.sh` 1개가 있으며, 각 root script는 inventory, lifecycle category, repository reference, 또는 explicit standalone exemption으로 설명되어야 한다.
...
- **In Scope**:
  ...
  - `docs/05.plans/2026-05-09-scripts-lifecycle-contract-cleanup.md`
  - `docs/05.plans/README.md`
...
| PLN-004 | Plan artifact 추가 | `docs/05.plans/2026-05-09-scripts-lifecycle-contract-cleanup.md` | REQ-DOC-001 | plan template 필수 섹션과 실제 related links 포함 |
| PLN-005 | Parent README sync | `docs/05.plans/README.md` | REQ-DOC-002 | 새 plan이 structure와 related documents에 연결됨 |
...
- [x] The new plan is linked from `docs/05.plans/README.md`.

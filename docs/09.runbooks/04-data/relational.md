...
- [ ] 현재 리더 노드 식별 (`patronictl list`)
- [ ] etcd 엔드포인트 도달 가능성 확인
...
#### 2. etcd 정족수 복구
1. 모든 etcd 노드 상태 확인: `etcdctl endpoint health`
2. 과반수 미만 가동 시 클러스터 재구성 (Backup 활용)

#### 3. 수동 페일오버 (Switchover)
1. 리더 교체가 필요한 경우: `patronictl switchover`
2. 대상 노드 선택 및 실행 컨펌

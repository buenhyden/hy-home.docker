...
`docs/07.operations/04-data/specialized`는 운영 정책, 통제 기준, 검증 방법을 정의하는 operation 위치다. 이 README는 하위 파일을 찾는 진입점이며, 기존 본문과 실제 디렉터리 구조를 함께 기준으로 사용한다.
...
```text
docs/07.operations/04-data/specialized/
├── neo4j.md  # 문서
├── qdrant.md  # 문서
└── README.md  # This file
```
...
> Migrated from `docs/07.operations/04-data/specialized/README.md` during the 2026-05-10 operations taxonomy consolidation.
...
`docs/07.operations/04-data/specialized`는 사용자와 운영자가 재현 가능한 작업 방법을 이해하도록 돕는 guide 위치다. 이 README는 하위 파일을 찾는 진입점이며, 기존 본문과 실제 디렉터리 구조를 함께 기준으로 사용한다.
...
- [Storage Exhaustion Procedure](../storage-exhaustion.md)
- [ARD](../../../02.ard/0004-data-architecture.md)
...
`docs/07.operations/04-data/specialized`는 운영자가 즉시 실행할 수 있는 runbook 위치다. 이 README는 하위 파일을 찾는 진입점이며, 기존 본문과 실제 디렉터리 구조를 함께 기준으로 사용한다.

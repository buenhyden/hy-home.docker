---
status: active
artifact_id: policy:llm-wiki-maintenance
...
이 가이드는 루트 `llms.txt`, `docs/90.references/llm-wiki/`, `scripts/knowledge/generate-llm-wiki-index.sh`, `doc-writer` 역할과 `knowledge-map-agent` 함수의 운영 절차를 다룬다.

...
database while the service may write. Follow the central [backup policy](../policies/0021-backup-and-restore.md), restore first to isolated storage, and record identity/chat preservation evidence before any replacement. A configuration rollback does not justify a database restore.
...
1. Disable new sessions and ingestion, record image/database versions and identity/chat/upload counts, stop Open WebUI, and create a consistent backup of the whole `/app/backend/data` volume plus the matching auth/OIDC secret references. Obtain the Qdrant snapshot reference from [RUN-0034](0034-qdrant.md); do not operate on its storage here.

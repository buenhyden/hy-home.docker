...
이 런북은 `k6-master` 실행 중 target service SLI가 급락하거나 InfluxDB 지표 전송 실패, healthcheck 실패가 발생했을 때 사용한다. 현재 k6 leaf는 별도 worker service가 없는 단일 Locust wrapper다.

### Purpose

테스트 부하를 안전하게 중단하고, root optional compose context와 metric sink 상태를 확인한 뒤 재실행 여부를 판단한다.
...
- InfluxDB write error 또는 token 관련 오류가 `k6-master` 로그에 나타난다.
...
- [ ] `influxdb_api_token` secret 이름만 확인하고 secret 값은 열람하지 않는다.
...
- **Logs**: `k6-master` logs, target service logs, InfluxDB write-error logs
- **Metrics**: target SLI, request error rate, response latency, InfluxDB bucket write status
...
Use the stop-and-review procedure above. No broader verified rollback procedure is documented for changing target services, InfluxDB data, or host networking from this runbook.

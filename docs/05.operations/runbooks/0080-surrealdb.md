...
- [ ] Record configuration commit, Dockerfile/[Compose](../../../infra/11-laboratory/open-notebook/docker-compose.yml) version authority and [runtime version projection](../../../infra/tech-stack.versions.json), engine version (must be SurrealDB v2 for Open Notebook), namespace, database, expected schema and record invariants.
...
3. Check reachability: `docker compose exec -T surrealdb /usr/local/bin/surreal is-ready --endpoint http://127.0.0.1:8000`.
...
3. Prepare a fresh compatible target running SurrealDB v2 (v3 is incompatible with Open Notebook) that shares no production port, network, volume, or secret. Create separate test credentials at the auth scope needed for import.
...
- Declared parent: [SurrealDB Policy](../policies/0080-surrealdb.md) (`POL-0080`)
- Governing architecture: [AD-0011](../../02.architecture/descriptions/0011-laboratory-architecture.md)
- Subject peers: [Guide](../guides/0080-surrealdb.md), [Policy](../policies/0080-surrealdb.md)

...
- 캐시 및 NoSQL 운영 정책 (-> `docs/05.operations/04-data/cache-and-kv/` 등)
- 데이터베이스 쿼리 튜닝 가이드 (-> `docs/05.operations/04-data/relational/` 참조)
...
1. 전반적인 운영 계획은 [Data Optimization Plan](../../../../04.execution/plans/2026-03-26-04-data-standardization.md)을 참조합니다.
2. 모든 운영 정책은 [Data Specification](../../../../03.specs/04-data/spec.md)의 기술 제약 사항을 준수해야 합니다.
3. 새로운 정책 정의 시 `docs/99.templates/operation.template.md`를 사용합니다.

## Usage Instructions

- 이 영역의 정책 문서는 정기적으로 검토되어야 하며, 인프라 변경 시 실시간으로 갱신되어야 합니다.
- 정책 위반 사례 발생 시 [Incident Records](../../../incidents/README.md)와 연계하여 원인을 분석합니다.
- 상세 실행 절차는 [Relational Procedures](./README.md)를 참조하십시오.

## Incident and Recovery Links

- **Usages**: [Relational Usages](./README.md)
...
> Migrated from `docs/05.operations/04-data/relational/README.md` during the 2026-05-10 operations taxonomy consolidation.
...
- NoSQL 및 캐시 서비스 가이드 (-> `docs/05.operations/04-data/nosql/` 등)
...
1. 전반적인 데이터 아키텍처는 [Data Spec](../../../../03.specs/04-data/spec.md) 문서를 먼저 확인합니다.
...
- 캐시 및 NoSQL 복구 절차 (-> `docs/05.operations/04-data/nosql/` 등)
- 데이터 아키텍처 및 상세 설계 명세 (-> `docs/03.specs/04-data/spec.md` 참조)
...
1. 장애 복구 시 [Operational Policy](./postgresql-cluster.md)를 먼저 확인하여 통제 및 승인 기준을 준수합니다.
2. 모든 실행 결과는 [Incident/Postmortem](../../../incidents/README.md) 문서화를 통해 학습 자산으로 남깁니다.

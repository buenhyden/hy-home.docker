...
2. 관리 UI는 `mongo-express`가 제공하며 Traefik route `https://mongo-express.${DEFAULT_URL}`를 사용한다. `ME_CONFIG_BASICAUTH=true`로 basic auth가 켜져 있어야 하며(mongo-express 1.x는 이 값 없이 자격 증명을 무시한다), 직접 host port publish는 현재 compose에 없다.

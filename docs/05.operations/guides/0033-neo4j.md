...
정상 운영 중 점검은 compose profile 렌더링, 서비스 상태, Traefik HTTP router 기준 Browser route(`https://neo4j.${DEFAULT_URL}`), container-local secret mount 기반 Bolt `RETURN 1` 확인으로 구성된다. 애플리케이션이 `edge_net` 내부에서 접근할 때는 `bolt://neo4j:${NEO4J_BOLT_PORT:-7687}`를 기준으로 한다. Public Bolt TCP route는 현재 compose에 선언되어 있지 않으므로 별도 gateway 변경 승인 없이는 문서화하지 않는다. 실행 가능한 명령 순서와 기대 결과는 [Neo4j runbook](../runbooks/0033-neo4j.md#steps)을 따른다.
...
반복 실행 절차, 장애 대응, rollback 또는 escalation 기준은 [Neo4j runbook](../runbooks/0033-neo4j.md)을 따른다.

## Traceability

- Declared parent: [Neo4j Operations Policy](../policies/0033-neo4j.md) (`POL-0033`)
- Governing authority: [Data Tier (04-data) Architecture Description](../../02.architecture/descriptions/0004-data-architecture.md) (`AD-0004`)
- Subject peers: [Policy](../policies/0033-neo4j.md) (`POL-0033`), [Runbook](../runbooks/0033-neo4j.md) (`RUN-0033`)
...
- [Operations policy](../policies/0033-neo4j.md)
- [Recovery runbook](../runbooks/0033-neo4j.md)

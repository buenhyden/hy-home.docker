...
updated: 2026-08-11
---
<!-- Target: docs/05.operations/catalog/11-laboratory/ops-0075-portainer/policy.md -->
...
- **Access Control**:
  - 반드시 Traefik `gateway-standard-chain@file,portainer-admin-ip@docker,sso-errors@file,sso-auth@file` 미들웨어 체인을 통해 1차 인증과 allowlist를 수행해야 한다.
  - Portainer 내부 RBAC를 활성화하여 불필요한 관리자 권한 부여를 제한한다.
- **Data Persistence**:
  - `portainer-data` 볼륨은 `${DEFAULT_MANAGEMENT_DIR}/portainer`에 매핑되어야 하며, 시스템 백업 대상에 포함되어야 한다.
- **Resource Management**:

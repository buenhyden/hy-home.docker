...
> Scope: Loki readiness checks, Alloy ingestion evidence, SeaweedFS storage/retention verification, restart, and config rollback.

이 런북은 Loki readiness failure, Grafana "no logs found", Alloy ingestion failure, SeaweedFS storage/credential symptom, retention/compactor issue, label cardinality regression, and config regression을 다룬다. Guide와 policy의 설명을 반복하지 않고 실행 가능한 진단, 안전한 restart, evidence capture, escalation 기준을 제공한다.

### Purpose

운영자가 `infra-loki` 상태를 확인하고 Alloy -> Loki ingestion, SeaweedFS S3 backend, retention/compactor config, custom image secret expansion, Grafana datasource를 검증하며, Secret 노출이나 retention/resource/runtime 변경 같은 위험 조치를 별도 승인으로 격리하도록 돕는다.
...
- `S3_ACCESS_KEY`, `seaweedfs_s3_loki_secret_key`, `loki-bucket`, retention, compactor, or label policy 변경 후 검증이 필요할 때.
...
- [ ] 문제 유형을 readiness, Alloy ingestion, Grafana datasource/query, SeaweedFS storage/credential, retention/compactor, label cardinality, config regression 중 하나로 분류한다.
- [ ] `S3_SECRET_KEY` value, SeaweedFS credential value, rendered environment, or secret payload 원문은 기록하지 않는다.
- [ ] Retention, resource cap, SeaweedFS bucket, route, or secret reference를 변경해야 해 보이면 중단하고 owning operator approval을 받는다.
...
4. SeaweedFS storage, retention, compactor, and ruler boundary를 확인한다.
...
6. Storage or retention symptom이 의심되면 Loki logs에서 관련 증상만 추출한다.
   ...
   ```bash
   docker logs --tail=500 infra-loki | grep -Ei 's3|bucket|loki-bucket|seaweedfs|access denied|secret|retention|compactor|rate limit|out of order|oom|error|warn'
   ```
...
8. `loki-config.yaml`, custom image, or entrypoint 변경 후 장애가 발생했다면 Git-managed diff를 되돌리고 readiness를 재확인한다.
   ...
   이 런북은 retention change, SeaweedFS bucket migration, resource cap increase, secret rotation, high-cardinality label expansion, or protected middleware change를 검증된 복구 절차로 제공하지 않는다. 해당 변경은 별도 approval과 rollback evidence가 필요하다.
...
- Retention, SeaweedFS bucket, resource cap, secret rotation, label cardinality policy, or protected route middleware change는 이 런북의 안전 롤백 범위를 벗어난다.
...
1. Record image/config/schema/retention values, bucket version/checksum inventory, local-state identity, and a bounded test-query baseline. Quiesce ingestion and coordinate a consistent `loki-bucket` snapshot with the SeaweedFS owner plus a stopped copy of recovery-relevant `loki-data`.
...
- Secret 값, rendered environment, SeaweedFS credential 원문은 기록하지 않는다.
...
이 런북에 명시된 validation, restart, and Git-managed config/image/entrypoint rollback만 사용한다. Retention, SeaweedFS bucket, resource cap, secret rotation, high-cardinality label, protected middleware, or external storage 변경은 검증된 안전 복구 절차가 아니므로 `## Escalation`으로 이동한다.

...
선택된 Compose 이미지의 drift 투영이다. 빌드 전용 서비스, Dockerfile의
상속 단계와 전체 의존성을 레지스트리가 완전히 나열한다고 해석하지 않는다.
...
확인한다. Green registry check만으로 Dockerfile pin 검증을 대체하지 않는다.

## Runbook Handoff

[Runbook](runbook.md)이 검증 순서와 rollback을 소유하며 [Policy](policy.md)가

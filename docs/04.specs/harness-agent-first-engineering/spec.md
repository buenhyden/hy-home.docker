...
이 문서는 `hy-home.docker` 워크스페이스의 목적, 규칙, 환경, 하네스 구성, Agent-first Engineering 구성을 파일 내용 기준으로 분석하고, 현재 구현이 충족하는 계약과 남은 보완 방식을 명세한다.
...
| Harness runtime | `.claude/CLAUDE.md`, `.claude/settings.json`, `.claude/agents/*.md`, `.claude/skills/*/skill.md` | supervisor는 `opus`, workers는 `sonnet`, agents는 단일 scope import를 가진다. |
...
- Agent/runtime changes are auditable through catalog parity and validation scripts.
...
| --- | --- | --- |
| Stage docs | Markdown with `status: draft` front matter | Human and agent-readable analysis, plan, task, guide, policy, and runbook. |
...
- Validators pass with zero failures.

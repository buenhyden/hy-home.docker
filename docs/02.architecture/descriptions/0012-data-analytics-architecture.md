...
본 문서는 `04-data/analytics` 서브 티어의 기술적 뼈대와 시스템 아키텍처를 정의한다. 핵심 데이터 티어로부터 분리된 분석 전용 하이퍼스케일 엔진들(InfluxDB, OpenSearch)의 배치 전략, 데이터 흐름, 그리고 플랫폼 통합 방식을 상세히 기술한다. 스트림 처리(Flink)와 OLAP 웨어하우징(Trino)은 `04-data/lakehouse`가 소유하며 이 설명의 범위 밖이다.
...
- **Scalability**: 데이터량 및 쿼리 부하에 따라 OpenSearch 노드 확장 가능성 보장.
...
현재 tracked compose는 analytics engines and endpoints를 제공한다. Kafka-to-engine ingestion and dashboard datasets are application or workflow concerns and are not wired as automatic data pipelines in the analytics compose files.
...
- **Networking**: 모든 서비스는 선언된 network를 사용하지만 호스트 노출 방식은 서비스별 Compose 선언을 따른다. OpenSearch와 OpenSearch Dashboards HTTP 경로는 Traefik을 사용한다. OpenSearch cluster의 node1은 Performance Analyzer `${ES_PERFORMANCE_ANALYZER_HOST_PORT:-9600}:${ES_PERFORMANCE_ANALYZER_PORT:-9600}`을 호스트에 게시한다. 이 포트들은 Gateway 전용 노출로 간주하지 않는다.
- **Storage Bindings**:
  - InfluxDB, OpenSearch, OpenSearch Dashboards는 bind-backed named volume을 사용한다.
  - 현재 compose의 device paths는 `${DEFAULT_DATA_DIR}/influxdb`, `${DEFAULT_DATA_DIR}/opensearch` 계열이다.
...
- **Query Optimization**: 에이전트는 비효율적인 분석 쿼리 패턴을 감지하고 OpenSearch 인덱싱 등의 최적화 제안 가능.
...
- **Ingestion**: 현재 compose는 ingestion endpoints and dependencies를 제공한다. Kafka CDC, OpenSearch indexing, and InfluxDB writes require separate producer/workflow evidence.
- **Storage Strategy**:

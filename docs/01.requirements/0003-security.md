...
`03-security` 티어는 하이홈 도커 플랫폼의 "Root of Trust" 역할을 하며, OpenBao를 통해 민감한 데이터(비밀번호, API 키, 인증서 등)를 중앙에서 관리하고 암호화한다. 모든 서비스는 소스 코드나 환경 변수에 비밀 정보를 직접 노출하지 않고, OpenBao를 통해 동적으로 주입받거나 참조하는 보안 체계를 구축한다.

### Problem Statement

서비스별 비밀 정보가 파일, 환경 변수, 수동 절차로 분산되면 노출 위험과 감사 공백이 커진다. `03-security`는 OpenBao를 중심으로 비밀 저장, 접근 제어, 주입, 감사 로그를 통합해 모든 서비스가 동일한 신뢰 경계를 따르게 해야 한다.

## Stakeholders and User Needs

- **보안성**: 모든 비밀 정보의 암호화 저장 및 접근 제어 자동화.
- **투명성**: 누가, 언제, 어떤 비밀 정보에 접근했는지에 대한 감사 로그 확보.
- **편의성**: OpenBao Agent를 활용한 사이드카 패턴으로 애플리케이션 수정 최소화.
...
| Infra Engineer | 플랫폼 운영 및 관리    | OpenBao 서버의 고가용성(Raft) 유지 및 서비스별 접근 권한 할당.          |
| App Developer  | 서비스 개발 및 배포    | 환경 변수 대신 OpenBao에서 동적으로 비밀 정보를 주입받는 메커니즘 제공. |
...
| UC-02 | Sidecar Injection | OpenBao Agent를 통해 컨테이너 시작 시 템플릿 기반으로 설정 파일에 비밀 정보 주입. |
...
| REQ-0003-FR-0002 | Forward Proxy Support | Traefik을 통한 OpenBao UI/API 외부 노출 및 SSL 적용. |
...
| ------ | ------------ | ---------------------------------------------------------------------- |
| REQ-0003-NFR-0005 | Availability | 단일 노드 OpenBao 장애 시 영향을 명확히 감지하고, 향후 Raft quorum 확장을 위한 운영 절차를 유지. |
...
- **REQ-0003-FR-0001**: 현재 OpenBao/Agent compose와 문서가 root `security`/`core` profile 검증 및 hardening gate로 관리됨.
- **REQ-0003-FR-0002**: 서비스 컨테이너가 직접 비밀 정보를 알지 못해도 OpenBao Agent를 통해 주입받아 성공적으로 구동됨.
- **REQ-0003-FR-0003**: 감사 로그/원격 audit 고도화는 정책 승인 후 단계적으로 전환됨.

## Risks

- **Risk**: OpenBao 쿼럼이나 Unseal 절차가 실패하면 비밀 정보 주입이 중단될 수 있음.
- **Dependency**: Traefik 기반 외부 노출과 Docker/OpenBao Agent 사이드카 패턴에 의존함.
- **Assumption**: 서비스별 secret path와 ACL 정책은 OpenBao KV 및 AppRole/Userpass 권한 모델로 관리됨.

...
| WDC-GAP-011 | `AGENTS.md`; `scripts/knowledge/report-graphify-health.sh` | Graphify refresh is instruction-based and report-only, not enforced as a freshness gate. | `docs/90.references/audits/document-contracts/automation-coverage-map.md:170` | `batch-fix` | Closed locally by the T-005 decision batch: Graphify remains advisory until CLI availability and graph health are stable enough for a hard gate. |

...
| QAF-04 | Run scoped static linting with explicit exclusions and duplicate-job ownership. | Markdown, YAML, shell, workflow, Dockerfile, secret, and Python-format hooks each declare an explicit `files` selector, and Storybook lint runs as its own gate leaf. Re-observed 2026-09-08, superseding the two examples this row first gave: `recommend-qa-gates.sh` was removed with the Task 3 wrappers, the ShellCheck hook declares no `exclude` at all, and ESLint has no pre-commit hook, so nothing skips it and `leaf.frontend-lint` owns it outright. The only exclusions in force are the repository-wide `graphify-out/` one and the frozen archive anchor the three text mutators share. `ruff-format` is registered but no `ruff check` or other Python lint owner is. | Partial | 3 | Improve | QA scope and language/project owners | Existing scoped hooks/jobs; inventory uncovered languages before expansion. | Inspect hook `files`/`exclude`, CI `SKIP`, and dedicated lint commands. | High. |
...
| QAF-11 | Preserve hook stage/file filters and report intentional skips. | Re-observed 2026-09-08, superseding the counts this row first gave: `.pre-commit-config.yaml` registers twenty hook IDs, eighteen at `pre-commit`, one at `pre-push`, and one at `commit-msg`, and thirteen carry an explicit `files`, `exclude`, or `stages` filter. The CI wrapper owns its skip list and rejects a caller-supplied one; it skips `public-validation-changed` and `public-validation-full`, the two hooks that invoke the gate, which is what stops the two orchestrators from re-entering each other. `tests/validation/test_run_ci_precommit.sh` pins that exact value, so the intentional skip is reported and checked rather than assumed. The earlier `eslint-nextjs` example named a hook ID that had already left the file. | Implemented | 3 | Retain | `.pre-commit-config.yaml` and CI workflow owner | Existing hook/CI orchestration; skipped hook is not silently counted. | Count hook IDs and inspect `files`, `exclude`, `stages`, and CI `SKIP`. | High. |

...
| LLM Wiki freshness | `scripts/knowledge/generate-llm-wiki-index.sh` | `docs/90.references/data/0082-llm-wiki-index/README.md` | Generator builds the tracked safe path index from `git ls-files` and excludes secret contents and generated dependency trees. | no-action |
...
| Legacy `updated` frontmatter remains in active Stage 05 documents | Task 2 inventory found `updated:` in `docs/05.operations/06-observability/ops-0043-loki/guide.md`, `docs/05.operations/policies/06-observability/01.retention.md`, and `docs/05.operations/06-observability/ops-0043-loki/policy.md`. | Frontmatter Contract treats generic `updated` as duplicate-purpose metadata unless a profile explicitly consumes it. | batch-fix |

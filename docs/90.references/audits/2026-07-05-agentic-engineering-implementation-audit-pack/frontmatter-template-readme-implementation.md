...
| DML-03 | Record only direct upstream artifact IDs as resolvable parents, with deterministic multiple-parent handling. | Migrated documents use `parent_ids`; the checker builds a deterministic ID manifest and enforces resolvable direct parents, permitted roots, ordering, self-reference, and cycles. T-AER-012 hardens impacted dependents across deletion and identity changes. | Implemented | 3 | Retain | Metadata checker and stage owners | Retain relation and referential-integrity fixtures for current, staged, and explicit-base changes. | T-AER-008/012 focused tests plus changed/new validation. | High for migrated/changed and impacted-dependent scope. |
...
| DML-09 | Derive README role from its profile unless a real metadata consumer is declared. | The checker infers a dedicated README profile and treats folder-index lifecycle fields as optional/profile-dependent; explicit metadata-consumer intent is not yet declared for every status-bearing README. | Partial | 2 documented/profile-dependent | Improve | Documentation protocol owner | Retain README profile inference and add consumer exception evidence only when a real consumer exists; do not bulk-delete or add status. | Generated profile inventory, checker tests, and README template review. | High for profile behavior; Medium for individual consumer intent. |

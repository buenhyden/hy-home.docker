...
| DML-13 | Preserve stable criterion rows and evidence rather than a composite score. | Spec 123 defines ten fields; this report and the SDLC report use one complete row per criterion. | Implemented | 2 documented and generator-supported | Retain | Canonical audit pack owner | Keep audit matrix/coverage generators fresh and retain row-level links. | Review table shape and generated audit outputs. | High. |

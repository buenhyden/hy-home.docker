...
| DML-13 | Preserve stable criterion rows and evidence rather than a composite score. | Spec 123 defines ten fields; this report and the SDLC report use one complete row per criterion. The current generated matrix omits both reports and all 36 rows because its input list still names the historical eight reports. | Partial | 2 documented; generator coverage incomplete | Improve | Canonical audit pack owner | Keep row-level evidence canonical; Task 6 adds both reports to generator ownership rather than hand-editing output. | Review the 14 DML plus 22 SDLC rows directly; matrix freshness proves only the historical subset until consolidation. | High. |

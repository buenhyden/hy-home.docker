...
- Provider semantic role-scope parity, provider hook parity, LLM Wiki
  ...
  implementation matrix snapshots, and the eval fixture freshness CI gate are
  now covered. The remaining highest-value gaps are required semantic eval
  scoring, vulnerability gating, SBOM generation, and provenance/attestation
  automation.

...
이 reference는 LLM Wiki의 본문 repository map이다. 추적 파일을 기준으로 탐색 경로를 제공하며, 정책 판단은 `docs/00.agent-governance/`, 운영 판단은 `docs/05.operations/`, 최신 runtime truth는 `infra/`, `scripts/`, registry JSON 파일, Docker Compose 파일에서 확인한다.
...
- public wiki site, generated wiki, `llms-full.txt`

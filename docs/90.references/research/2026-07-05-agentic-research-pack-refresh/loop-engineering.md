...
| LOOP-03 — Validation and eval | Hooks and agents can invoke tests; provider capability does not adopt a scorer. | Agents/skills/hooks can invoke local checks and eval tooling. | Headless/tools/hooks/subagents can invoke checks. | Changed-file validation, CI/local routing, eight exact fixtures, ten synthetic regressions, deterministic scorers, calibrated thresholds, task evidence, and independent review define exit evidence. | The repository-semantic loop is implemented; no live provider-quality baseline is claimed. |

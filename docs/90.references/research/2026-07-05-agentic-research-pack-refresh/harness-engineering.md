...
| HAR-04 — Lifecycle interception  | Command, HTTP, prompt, MCP-tool, and agent handlers now span 31 documented event names, 15 of which can block; only the agent handler is marked experimental.           | Command hooks now document 11 events, including `SessionEnd` and `SubagentStart`. Interception still covers shell, patch, and MCP paths and still excludes hosted tools.                                                           | Gemini CLI documents 11 synchronous command-hook events across tool, agent, session, model, compression, tool-selection, and notification events.     | A typed seven-event contract renders seven Claude, six Codex, and seven Gemini native mappings.                        | The Codex mapping gap is now repository-side, not provider-side: `SessionEnd` became available upstream and the typed binding was not extended. Repository parity is validator-backed, but no tracked configuration proves live interception.                               |
...
| HAR-06 — Models and reasoning    | Agent definitions can select model/`inherit` and set `effort` (`low` through `max`), which overrides the session value; there is no per-subagent thinking setting.                                              | Agent TOMLs can set exact model and `model_reasoning_effort`. The two OpenAI surfaces diverge: the Codex product ladder is Low, Medium, High, Extra high, Max, and Ultra with no `none`, while the API exposes `none` through `max` with no `ultra`.                                                  | CLI model selection and settings-level or API thinking controls exist, but the subagent file schema documents no reasoning or thinking field, and Antigravity selection is a separate surface.                          | `subagent-protocol.md` owns five exact profiles across the 11-model registry; there is no active fallback graph or implicit substitution.                            | Catalog presence and capability prose do not prove account availability, task quality, cost, or cross-provider equivalence. Codex effort values are surface-specific and must not be copied between the product and the API. |

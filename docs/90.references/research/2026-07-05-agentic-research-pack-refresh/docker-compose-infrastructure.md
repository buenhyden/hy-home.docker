...
The census was recomputed on `2026-07-19` from `git ls-files`, safe YAML parsing,
the root include block, and the canonical generated coverage. The generator
check passed; none of the prior 17/47/40 observations was copied without this
recount.
...
| Service definitions with `depends_on` | 94 / 168 | Static key presence; condition semantics vary by service. |
...
| Service definitions with `secrets` | 111 / 168 | 42 of the 60 entries in root-included leaves declare secret access. |
...
- The exact Docker Compose include, profiles, secrets, and trust-model pages and
  ...
  confirmed in that bounded set. Other lower-risk sources retain their earlier

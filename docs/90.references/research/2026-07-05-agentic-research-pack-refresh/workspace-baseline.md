...
| AI agents | Stage 00 owns 14 roles (one supervisor and thirteen workers), 22 functions, and a typed seven-entry external capability-intake ledger. Four role surfaces each contain 14 adapters; function skills project to Claude and shared `.agents` only. | The pinned `agency-agents` repository and official provider agent schemas show broader catalog and native-agent patterns without proving workspace suitability. | Implemented for canonical intake/projection | Live provider acceptance and live comparative role/model evaluation remain absent. | Retain merge/defer decisions, source/license provenance, generated-only adapters, synthetic evaluation, and independent review for future candidates. | `docs/90.references/research/2026-07-05-agentic-research-pack-refresh/ai-agent-catalogs.md` | High for tracked definitions |
...
| task-characteristic model selection | The typed policy implements supervision Opus 4.8/GPT-5.6/Gemini 3.5, complex Sonnet 5/GPT-5.6/Gemini 3.5, and read-heavy Haiku 4.5/GPT-5.6 Terra/Gemini 3.1 Flash-Lite profiles; the historical landscape remains 145 structural / 142 cutoff-qualified rows. | Official provider catalogs and capability sources establish cutoff evidence and provider-native controls, not account entitlement or benchmark parity. | Partially Implemented | GPT-5.6 is listed but not provider-labeled Stable and remains cutoff-unverified; all live entitlements and cross-provider task-quality/latency/cost equivalence remain unproven. | Preserve typed evidence states and require a separate approved live-evaluation/runtime task before claiming availability or equivalence. | `docs/90.references/research/2026-07-05-agentic-research-pack-refresh/agent-model-selection.md` | High for tracked policy |
...
- [GitHub Docs content best practices](https://docs.github.com/en/contributing/writing-for-github-docs/best-practices-for-github-docs) and [Diataxis](https://diataxis.fr/) - audience/purpose and content-type separation

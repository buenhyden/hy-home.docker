...
| --- | --- | --- |
| Stage 01-05 lifecycle leaves | 531 | 25 requirements, 50 architecture, 30 Spec-stage, 234 execution, and 192 operations leaves. |
| Lifecycle statuses in those leaves | 294 `active`, 235 `completed`, 2 `draft` | Status exists across the measured active-stage corpus; `completed` remains durable evidence, not archive. |
...
| Current role paths | PRD 25; ARD 25; ADR 25; Plan 103; Task 131; Guide 66; Policy 64; Runbook 62 | Role counts are derived from canonical paths. Legacy `artifact_type` coverage is incomplete and cannot replace path measurement. |

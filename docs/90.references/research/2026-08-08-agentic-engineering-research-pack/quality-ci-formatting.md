...
coverage apply to the Storybook Next.js project only. Python has 22 tracked
`unittest` files, but no tracked Ruff/Black/Flake8/Pylint lint gate, no
Mypy/Pyright type gate, and no Python coverage gate. These are partial states,
not whole-repository quality claims.
...
| Python unit/regression | 22 tracked `tests/validation/test_*.py` files using `unittest` | Selected modules in local typed roots; direct module/discovery commands remain possible | Selected modules in repo-contract, eval, and supply-chain roots | **Partial.** The typed roots do not claim one full 22-file discovery run. |
...
| [CI gate contract tests](../../../../tests/validation/test_ci_gate_contract.py) | 2026-08-08 | Workspace tracked | The tracked validation directory contains 22 Python and 2 shell test files; file presence is not a full-suite result. |

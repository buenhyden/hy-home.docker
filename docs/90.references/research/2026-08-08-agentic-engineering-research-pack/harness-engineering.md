...
| --- | --- | --- |
| `permissions.allow`/`deny`/`ask`, merged across scopes (any-scope deny wins; allow accumulates) | `allow`: 8 `Bash(...)` prefixes (`git`, `python3`, `grep`, `rg`, 4 read-only docker/compose, 5 named validators); `deny`: 4 destructive prefixes (`docker system prune`, `rm -rf`, `docker compose down`, `docker volume rm`) | No `ask` entries; every command runs unprompted or is denied. `.claude/settings.local.json` (git-ignored, not part of the tracked corpus) adds a small personal `allow` list. |

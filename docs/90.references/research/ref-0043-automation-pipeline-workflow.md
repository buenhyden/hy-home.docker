...
<!-- Target: docs/90.references/research/ref-0043-automation-pipeline-workflow.md -->
...
machine identity. That file is explicit that it is a _proposal_: it applies no
...
This section inventories what the repository declares. The platform rules those
declarations sit inside — token scoping, OIDC, reusable workflows, script
injection, cache poisoning, rulesets, runner models — and the list of platform
capabilities the repository does not adopt are in
...
| Deployment rollback       | Stage 05 release readiness requires affected recovery links and backup/N/A evidence; no generic automated rollback is tracked.                                                                               | OWASP SAMM Secure Deployment expects repeatable deployment, security milestones, deployment records, and stop/reverse behavior for unacceptable defects. | Partially Implemented | Documentation coverage does not prove tested application/data rollback.                                         | Route service-specific rollback, migration compatibility, and recovery verification to the later runtime/delivery specifications.                           | `docs/05.operations/00-workspace/ops-0009-release-management/runbook.md` | Release runbook and service runbooks                                                                            | Medium     |

...
<!-- Target: docs/90.references/research/ref-0049-loop-engineering.md -->
...
- All four carry `runtime_depth: repository-enforced`, which is the only value
  ...
  No semantic-event binding carries it: 20 of the 21 are
  `configured-not-executed` and the Codex `session-end` binding is
  `unsupported`. The contract therefore already separates
...
- [harness engineering](ref-0047-harness-engineering.md)

...
Non-README reference 문서는 다음 조건을 만족해야 합니다.
...
README 파일은 `docs/99.templates/templates/common/readme.template.md`의 기본 구조를 따르고, 하위 reference 문서를 찾기 위한 index 역할을 합니다. README lifecycle은 기본적으로 path, heading, folder-index profile에서 파생되며, `status` frontmatter를 새로 추가하는 것이 필수는 아닙니다.
...
- Folder README는 frontmatter status 없이도 유효한 index surface이며, 이미 존재하는 status frontmatter는 해당 README profile이 소비할 때만 유지한다.
...
| `data/governance/` | Governance routing, validation-reference data, reviewed lifecycle manifests, and generated safe summaries | README는 한국어 기본, non-README reference 문서는 closed-surface contract에 맞춰 영어 사용 |
...
| `data/security/` | Generated security automation readiness reference data | 한국어 README 기본, generated readiness snapshot은 machine-readable English 허용 |
...
- [data/governance/document-corpus-lifecycle/foundation.yaml](./data/governance/document-corpus-lifecycle/foundation.yaml) - reviewed advisory Foundation migration manifest

...
> audits, data, research 3축으로 관리하는 stable reference stage

## Overview

`docs/90.references`는 active stage 문서를 보조하는 안정적인 참고 지식 공간입니다. 이 stage는 요구사항, 아키텍처, 명세, 실행 계획, 운영 문서가 반복해서 참조하는 배경 지식과 기준 정보를 `audits`, `data`, `research` 세 축으로 관리합니다.
...
├── data/         # Stable reference data, glossary, profiles, generated indexes, and inventories
...
- Top-level folder는 `audits/`, `data/`, `research/` 중 하나만 사용한다.
...
| Stable background, glossary, generated index, profile, inventory, learning map | `docs/90.references/data/` |
...
| `data/learning/` | CS/CE/SE learning roadmap and theory references | 한국어 학습 설명 기본, 논문/책/표준명 원문 보존 |
| `data/llm-wiki/` | Repo-local LLM navigation map and generated tracked path index | README는 한국어 기본, generated `index.md`와 machine-readable navigation text는 English 허용 |
...
- [data/learning/README.md](./data/learning/README.md) - Docker-based infrastructure learning roadmap and theory references
- [data/llm-wiki/README.md](./data/llm-wiki/README.md) - repo-local LLM Wiki entrypoint and tracked-source repository map
- [data/llm-wiki/repository-map.md](./data/llm-wiki/repository-map.md) - curated tracked-source repository map
- [data/llm-wiki/index.md](./data/llm-wiki/index.md) - generated tracked repo-local path index
...
3. Choose `audits/`, `data/`, or `research/` before creating a subcategory.

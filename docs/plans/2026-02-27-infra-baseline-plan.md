...
This plan operationalizes the baseline invariants defined in `specs/infra/baseline/spec.md`, aligned with `docs/prd/2026-02-27-infra-baseline-prd.md`.
...
- **Requirements:**
  - `[REQ-INFRA-BSL-001]`: All images MUST use pinned version tags; `latest` is prohibited (maps to `REQ-SPC-BSL-01`).
  - `[REQ-INFRA-BSL-002]`: Every service SHALL utilize `init: true` for signal handling (maps to `REQ-SPC-BSL-02`).
  - `[SEC-BSL-001]`: Sensitive data MUST be injected via `/run/secrets/` filesystem mounts (maps to `REQ-SPC-BSL-03`).
  - `[AC-BSL-001]`: (from `STORY-BSL-01`) Given a clean Docker environment, when running `preflight-compose.sh`, then missing secrets are identified.
- **Constraints:**
...
| TASK-004 | Ensure contributors can identify missing bootstrap prerequisites (env/secrets/certs/dirs) via preflight script. | `scripts/preflight-compose.sh`, `scripts/README.md`, `runbooks/infra-bootstrap-runbook.md` | [AC-BSL-001] | Running `bash scripts/preflight-compose.sh` reports missing prerequisites with non-zero exit (or WARN where documented). |

**Traceability Matrix**

| Spec / PRD Item | Plan Task(s) | Verification(s) |
| --------------- | ------------ | --------------- |
| `REQ-SPC-BSL-01` | `TASK-001` | `VAL-BSL-PLN-002` |
| `REQ-SPC-BSL-02` | `TASK-002` | `VAL-BSL-PLN-001` |
| `REQ-SPC-BSL-03` | `TASK-003` | `VAL-BSL-PLN-001` |
| `STORY-BSL-01` | `TASK-004` | `VAL-BSL-PLN-003` |
...
- **ARD**: `docs/ard/2026-02-27-infra-baseline-ard.md`

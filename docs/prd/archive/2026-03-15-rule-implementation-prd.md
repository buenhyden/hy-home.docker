...
- **[REQ-RULE-FUN-01] Uniform Rule Triggers**: `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` must share a consistent trigger mechanism (e.g., `[LOAD:RULES:<NAME>]`).
- **[REQ-RULE-FUN-02] Lazy Loading Alignment**: Rule triggers must point to modular files in `docs/agentic/rules/`.
- **[REQ-RULE-FUN-03] Skill Autonomy Mandate**: Root agent files must explicitly state that no skills are restricted.
- **[REQ-RULE-FUN-04] Required paths must align with category taxonomy.**:
  - ARD: `docs/ard/`

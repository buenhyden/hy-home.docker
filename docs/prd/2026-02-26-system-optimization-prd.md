---
title: '[PRD-OPT-01] Hy-Home System Optimization'
status: 'Approved'
version: 'v1.0.0'
...
| ------------------ | -------------------- | ------------------ | ---------------- | ------------------- |
| **REQ-PRD-OPT-MET-01** | Security Audit Score | 50%                | 100%             | Continuous          |
| **REQ-PRD-OPT-MET-02** | Observability Index | Mix of JSON/Plain  | 100% Loki/Tiered | Audit cycle         |
| **REQ-PRD-OPT-MET-03** | Cold Startup Time    | > 30s              | < 15s (Gateway)  | Startup cycle       |
| **REQ-PRD-OPT-MET-04** | Portability Index    | 5+ Static IPs      | 0 Static IPs     | Registry check      |

## 4. Key Use Cases & Acceptance Criteria (GWT)

| ID           | User Story (INVEST)                                                                      | Acceptance Criteria (Given-When-Then)                                                                                                |
| ------------ | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **STORY-OPT-01** | **As a** Developer,<br>**I want** consistent security defaults,<br>**So that** I don't misconfigure services. | **Given** a new `docker-compose.yml`,<br>**When** extending `base-security`,<br>**Then** `cap_drop: ALL` is applied automatically. |
| **STORY-OPT-02** | **As an** Architect,<br>**I want** ordered startup,<br>**So that** services don't crash on boot. | **Given** a multi-tier stack start,<br>**When** Gateway depends on IdP health,<br>**Then** Gateway starts only after IdP is healthy. |

## 5. Scope & Functional Requirements

- **[REQ-PRD-OPT-FUN-01]** Standardized YAML anchors for security baselines.
- **[REQ-PRD-OPT-FUN-02]** Every service MUST use Loki logging with job-specific labels.
- **[REQ-PRD-OPT-FUN-03]** Service startup MUST be ordered via `service_healthy` conditions.
- **[REQ-PRD-OPT-FUN-04]** Consolidate orchestration using Docker Compose `include`.
- **[REQ-PRD-OPT-FUN-05]** Rely on internal DNS for all container networking.
- **[REQ-PRD-OPT-FUN-06]** Enforce `init: true` for zombie reaping and signal handling.
...
- **Dependencies**: RELIES on REQ-SPC-BSL-01 for global inheritance rules.

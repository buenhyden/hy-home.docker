...
- **Standard**: Mandatory alignment with `docs/00.agent-governance/policies/quality-standards.md`.
...
@import docs/00.agent-governance/roles/infra.md
# IaC pattern — validate → change → verify
# SLO: LATENCY < 200ms · Network: infra_net · Secrets: Docker Secrets only
```

Spawn via the active runtime's delegated-agent facility. Pass `roles/infra.md` path in context. Do not embed policy inline.

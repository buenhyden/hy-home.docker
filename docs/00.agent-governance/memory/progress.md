...
| --- | --- | --- | --- | --- | --- |
| 2026-05-10 | Agent event hook dispatcher | Done | Implemented `scripts/agent-event-hook.sh`, connected Claude wrappers and Codex hook config to `SessionStart`, `PreToolUse`, and `PostToolUse`, and enforced the event hook contract in repository validation. | N/A | Hook event simulations for SessionStart/PreToolUse/PostToolUse; `bash scripts/check-repo-contracts.sh`; `bash scripts/check-doc-traceability.sh`; LLM Wiki freshness check |
| 2026-05-10 | Hook validation gap closure | Done | Audited tracked Claude/Codex hook surfaces and strengthened provider-neutral PostToolUse validation for `llms.txt`, apply-patch move payloads, and `scripts/lib/*.sh` syntax coverage. | N/A | `bash -n .claude/hooks/*.sh scripts/*.sh scripts/lib/*.sh`; JSON hook config validation; simulated `scripts/post-tool-validate.sh` payloads; repo contract and doc traceability checks |
| 2026-05-10 | GitHub remote enforcement boundary | Done | Separated agent protected-branch discipline from verified remote ruleset enforcement, kept the local main-protection proposal explicit, and enforced CI job/ruleset status-check synchronization in repo contracts. | N/A | `git diff --check`; `bash scripts/check-repo-contracts.sh`; `bash scripts/check-doc-traceability.sh`; `bash scripts/generate-llm-wiki-index.sh --check`; `bash scripts/report-graphify-health.sh` |

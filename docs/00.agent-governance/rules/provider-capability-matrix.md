...
| Rules              | `rules/*.md` + `docs/00.agent-governance/rules/hooks/hookify.*.md` | canonical Hookify rules + `settings.json`; local projection not tracked | governance + `.codex/hooks.json`            | governance + `.gemini/settings.json` when rendered |

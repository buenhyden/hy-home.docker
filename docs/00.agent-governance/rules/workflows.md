...
- **CI/CD Pipeline**: `deployment-pipeline-design` / `deployment-procedures` → `ci-cd-engineer` execution → deployment reports.
...
- **Skill Development**: `writing-skills` / `skill-creator` → `skill-creator` execution → new reusable skill.
- **Hook Development**: `writing-hookify-rules` / `hook-development` → `hook-developer` execution → new `.local.md` hook.
- **Governance Configuration**: `update-config` → `rules-engineer` execution → policy updates.

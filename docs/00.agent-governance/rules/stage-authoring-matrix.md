...
| Operations | `docs/05.operations/catalog/<domain>/ops-####-<subject>/{guide,policy,runbook}.md` | Operations role templates |
...
Task 10B Operations convergence additionally uses the exact `mig-0002` subject
and file records. Its `approval.status: pending` is non-executable; structural
and semantic modes require explicit approval of the displayed 77-row table.

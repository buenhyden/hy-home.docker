...
| --- | --- | --- | --- | --- |
| 00 | `agent-governance` | policies, roles, provider adapters, skills | Stage 00 plus provider registry | contract, renderer parity, Task |
...
| 99 | `templates` | document contracts | registry, schemas, templates | registry/schema tests |

A `layer` value is the stage directory name without its numeric prefix. A
template source declares the layer of the stage it authors into, not `templates`.

...
README는 typed leaf metadata 예외다. 복사한 대상에서는 template-source
`status: draft` frontmatter를 제거하고 `artifact_id`, `artifact_type`,
`parent_ids`를 추가하지 않는다.
...
- Repo-local document links must be Markdown links relative to the target README path.
- Do not use absolute filesystem paths, `file://` URIs, or paths calculated from `docs/99.templates/`.
...
- Folder index README는 하위 문서나 서비스의 routing/index 책임을 설명한다.
- Service leaf README는 단일 서비스나 runtime component의 config/compose/operations evidence를 설명한다.
...
Folder index README와 service leaf README를 구분한다.

- Folder index README: 하위 디렉터리나 문서를 찾기 위한 구조, routing, lifecycle, parent/child 갱신 규칙을 우선한다.
...
- `incidents/YYYY/INC-###-<incident-title>/INC-###-<incident-title>.md` 는 장애 또는 이상 상황에서 **무슨 일이 발생했는지**, **영향 범위가 무엇인지**, **즉시 어떤 대응을 했는지**를 기록한다.
- `incidents/YYYY/INC-###-<incident-title>/postmortem.md` 는 사건 이후 **근본 원인**, **기여 요인**, **재발 방지 조치**, **학습 내용**을 정리한다.

...
| Task | What was attempted, changed, validated, reviewed, committed, deferred, or blocked? | Approved plan work begins or its disposition changes. | Reviewers and later operational work consume auditable execution evidence. |
...
a stable tracking identity and verification ownership.
...
- An Incident may start as a root record when no verified Runbook parent exists;
  ...
  Postmortem remains a strict child of its paired Incident. Do not invent a
...
- Supersession preserves direction and evidence. Do not rewrite an ADR,
  incident, or task record merely to make history resemble current guidance.

## Delivery and Deployment Boundary

The repository does not maintain a separate Release document role. Delivery
history remains with the executing Task, `CHANGELOG.md`, immutable Git tags,
artifact attestations, and the applicable deployment Runbook evidence. Runtime,
promotion environments, protection rules, secrets, and rollback execution
remain in their separately approved technical and operational chain.
...
contracts, Plan, Task evidence, and Operations. It is a set
...
Runbook; and delivery outcomes can create a follow-up Plan and Task.

...
| Archive | Preserve a completed change packet, concise provenance tombstone, or migration record outside the active chain. | `docs/98.archive/**` is the sole archive target and selects one typed stable archive role; the archive and retention contract owns its identity, field, and preservation boundaries. |
...
and consult the registry and current co-located Task scope. Do not infer a profile from

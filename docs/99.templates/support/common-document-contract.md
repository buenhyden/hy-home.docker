...
| Archive | Preserve a concise provenance tombstone for content or an active document removed from the current chain. | Root `archive/**` selects `content-archive`; `docs/98.archive/**` selects `sdlc-archive`. Both keep semantic `artifact_type: archive`, while the archive and retention contract owns their distinct field and preservation boundaries. |

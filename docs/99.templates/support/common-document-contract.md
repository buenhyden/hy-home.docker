...
| Archive | Preserve a concise provenance tombstone for an active document removed from the current chain. | `docs/98.archive/**` is the sole archive target and selects `sdlc-archive`; the archive and retention contract owns its field and preservation boundaries. |

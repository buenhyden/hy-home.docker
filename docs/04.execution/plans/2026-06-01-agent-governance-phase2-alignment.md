...
- Normalize all documented template deviations across the target stages.
- Redesign provider parity around a Stage 00 canonical adapter model.
...
## Phase 1 Residual Gap Inventory

| Gap | Evidence | Phase 3 Planning Impact |
| --- | --- | --- |
| GAP-001 Stage 00 concept map is incomplete | Core concept definitions are distributed across Stage 00 rather than centralized for the full requested vocabulary. | Add a Stage 00 concept map and make provider docs reference it instead of redefining terms. |
| GAP-002 Codex harness target conflicts with current policy | Current `providers/codex.md` rejects `.toml`, while the confirmed target requires `.codex/agents/*.toml`. | Replace the current Codex harness rule with a migration path to `.toml` plus compatibility handling for existing `.md` agents. |
| GAP-003 Model Policy lacks enforced reasoning effort | Current Codex agent frontmatter has `model` but no `model_reasoning_effort`. | Define strict 2026-05-29 model and reasoning-effort support before adding `.toml` enforcement. |
| GAP-004 Template/status contract is inconsistent | Documentation protocol, template README, and actual target-stage statuses do not fully agree. | Full template normalization must reconcile allowed statuses, required headings, aliases, and historical exceptions. |
| GAP-005 Legacy and stale plan content remains | A legacy AI governance reorg plan contains stale template text and goals that conflict with current provider structure. | Normalize or retire legacy plan deviations as part of the full template cleanup. |
| GAP-006 Markdown fence issues are not caught by validators | Some runtime agent files contain malformed closing fences labeled as text blocks. | Add validator or manual gate coverage before claiming provider-surface correctness. |
| GAP-007 Function and runtime skill catalogs are uneven | Governance function catalog entries are much thinner than runtime skill definitions. | Define catalog depth requirements and link functions to runtime skills without duplicating provider policy. |
| GAP-008 Provider parity model is architecture-coupled to Claude | Current parity text treats Claude as canonical runtime implementation. | Redesign Stage 00 as the canonical runtime catalog and convert provider surfaces into adapters. |
| GAP-009 Graphify is advisory, not authoritative | Graphify health reports surprising cross-root inferred edges. | Use Graphify only for navigation; corroborate architecture claims against tracked Stage 00 and stage docs. |
| GAP-010 Validation pass does not prove semantic completeness | Existing checks pass while several policy, template, and harness gaps remain. | Expand validators and manual gates to cover the selected full-normalization and adapter-redesign scope. |

## Decision Gates

These gates capture confirmed user decisions before Phase 3 implementation. Phase 3 must implement only the approved decision set below unless the user approves a later plan amendment.
...
| DG-004 | Confirmed | Normalize all template deviations rather than limiting remediation to active/high-risk deviations or exception-only documentation. | Expands Phase 3 document work across `docs/01`-`docs/05`, `docs/90`, and `docs/99`; requires stronger batching and validation evidence. |
| DG-005 | Confirmed | Redesign provider parity around a Stage 00 canonical adapter model. | Replaces the Claude-canonical mirror assumption with a Stage 00 canonical runtime catalog and provider adapters for Claude, Codex, and Gemini. |
...
- **Goals**:
  ...
  - Plan full template-deviation normalization across the target documentation stages.
  - Plan the provider parity redesign so Stage 00 becomes the canonical runtime catalog and each provider becomes an adapter.
...
- **In Scope**:
  ...
  - `.claude/**`, `.agents/**`, provider parity docs, runtime catalog docs, and sync/validation scripts when required by the Stage 00 canonical adapter redesign
...
- **Non-goals**:
  ...
  - Do not perform the full template normalization or provider adapter migration during Phase 2.
- **Out of Scope**:
...
| Template Contract | Template mapping exists, but status vocabulary and some template deviation handling are inconsistent. | Template Contract defines required headings, allowed variants, status vocabulary, and full deviation normalization requirements. |
...
| Provider Parity | Claude canonical runtime mirror model is active. | Stage 00 becomes the canonical runtime catalog; Claude, Codex, and Gemini become provider adapters generated or validated against that catalog. |
...
| --- | --- | --- | --- | --- |
| PLN-001 | Encode confirmed user decisions as Phase 3 constraints. | This plan; future paired task | G1a | DG-001 through DG-005 are fixed plan constraints; no high-impact decision remains open. |
...
| PLN-004 | Plan full Template Contract normalization. | `docs/99.templates/**`, `rules/documentation-protocol.md`, `rules/stage-authoring-matrix.md`, target-stage docs | TPL-CONTRACT | Status vocabulary, required headings, accepted aliases, and every known deviation are normalized rather than silently excepted. |
...
| PLN-007 | Plan Stage 00 canonical adapter redesign. | `docs/00.agent-governance/agents/**`, `providers/*.md`, `.claude/**`, `.codex/**`, `.agents/**`, sync scripts, validators | PROVIDER-ADAPTERS | Stage 00 owns the canonical runtime catalog; provider surfaces are generated, validated, or documented as adapters. |
| PLN-008 | Plan validation blind-spot coverage. | `scripts/validation/check-repo-contracts.sh`, supplemental manual checks, task evidence | QA-BLIND-SPOTS | Markdown fence issues, status vocabulary drift, legacy plan skip, template-deviation normalization, adapter parity, and model/reasoning fields have checks or explicit manual gates. |
| PLN-009 | Plan Phase 3 traceability artifacts. | Future task doc, plans/tasks README entries if required, `progress.md` | TRACE | Phase 3 has a paired task record with commands, results, deviations, and approval evidence. |
...
| VAL-PLN-008 | Template Normalization Evidence | Prove all target-stage template deviations are normalized | Repository contract checks plus a deviation inventory generated before and after Phase 3 | No undocumented deviation remains. |
| VAL-PLN-009 | Provider Adapter Evidence | Prove provider surfaces conform to Stage 00 canonical adapter model | Provider sync/validation scripts updated for Stage 00 canonical catalog | Claude, Codex, and Gemini surfaces match the Stage 00 catalog or fail with actionable drift. |
...
| Full template cleanup expands into a broad rewrite of hundreds of docs. | High | Batch by template type, run validators after each batch, and preserve historical meaning while normalizing structure. |
| Provider adapter redesign accidentally forks governance. | High | Make Stage 00 the only canonical runtime catalog; provider files may adapt format but not redefine policy. |
...
- **Human Approval Gate**: Required before Phase 3 implementation because all decision gates are now confirmed and the resulting scope is broad.
...
- [x] DG-001 through DG-005 answered by the user and reflected as plan constraints.

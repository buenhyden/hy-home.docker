...
finding recorded below, and the operative gate is now the fix-4 gate stated at
...
predict their verdicts. One fresh independent specification reviewer and one
fresh independent Python/security reviewer must each return C0/I0/M0 over that
one-file range before recovery round 4 may begin. No Plan-only fix consumes a
Step 0e implementation round, so the implementation count remains three of
five. Do not begin recovery round 4 or edit the Task, helper, or tests until
both fix-4 re-reviews return C0/I0/M0. Package construction, package or ref
consumption, Phase A, evidence-ref publication, real-index staging, deletion,
lifecycle mutation, Task 12, remote actions, and push all remain closed.
...
3. `docs/04.execution/tasks/2026-08-08-agentic-research-pack-rebuild.md` only
   for the actual immutable Plan correction/fix-1/fix-2/fix-3/fix-4 commit
   ranges and all five independent review/re-review pairs, the round-3
   implementation
...
- State the funnel scope exactly so the closure claim above is true as
  ...
  create cannot open a pre-existing special file at all, and a directory open
  and a `scandir` walk were measured non-blocking under every substitution
  prepared for them. The bundle writer's `/tmp` directory open, its exclusive
  create, the anonymous `memfd` descriptor, and the worktree-registry
  directory walk are therefore named here as out of funnel scope rather than
  routed through a funnel described as a reader. They keep their existing
  `O_NOFOLLOW`, exclusivity, and identity checks.
...
- Give funnels 2 and 3 the descriptor bound, stated in terms a non-spawning
  ...
  the descriptor, fail closed, and send no signal and reap nothing. Name the
  deadline mechanism rather than implying it: use `poll()` on the nonblocking
  descriptor against a monotonic deadline, or `signal.setitimer` with a
  handler that raises. A bare `subprocess`-style timeout keyword does not
  exist for these sites, and PEP 475 retries an interrupted syscall unless the
  handler raises, so an unmechanized cap is not a cap.
- Qualify what `O_NONBLOCK` buys. It prevents a FIFO or socket open from
  parking the process, which is the measured hazard here. It is not a general
  non-parking guarantee: a character or block device driver may block inside
  `open` irrespective of it, and real non-symlinked device behavior at these
  paths was not determined in the measurement this contract is built on.
  The guarantee is therefore carried jointly by `O_NONBLOCK`, the
  `fstat`-before-read type check, and the named deadline — not by
  `O_NONBLOCK` alone.
...
- Diagnose an evidence-ref lock residue and stop; never clear it.
  ...
  block, so a run stopped by the funnel bound can leave a zero-byte orphan
  lock inside the evidence namespace, and the helper's existing logic
  misreports that residue as `FOREIGN_REF`. Diagnose the residue distinctly as
  `STALE_REF_LOCK`; `FOREIGN_REF` stays reserved for a genuinely foreign ref.
  Raw loose enumeration must classify any `<canonical-leaf>.lock` sibling
  inside the fixed namespace as this distinct state instead of rejecting it as
  a name not admitted by `EVIDENCE_REF_PATTERN`.
...
- Because the gate performs no clearance, it does not need to and cannot
  establish ownership of the lock, and it makes no ownership claim. A lock
  left by this gate's own terminated attempt, a lock held by a live concurrent
  writer, and a same-UID lock planted with a backdated modification time are
  indistinguishable to it and are all the same terminal state. No attempt
  ordinal, age threshold, own-children-reaped condition, or provenance record
  is used to discriminate them; the fix-3 attribution machinery is removed
  rather than refined, because its only consumer was the clearance it
  authorized.
- The diagnosis is read-only and bounded: an `lstat` of the exact `.lock`
  sibling discovered by the descriptor-relative raw walk. Report exactly what
...
- Exit status for `STALE_REF_LOCK` is 2, with empty stdout and stderr-only
  diagnostics, matching every other fail-closed code in this contract. Any
  ambiguity — an unreadable `.lock` sibling, a non-regular one, a `.lock`
...
- Within each complete namespace snapshot, take the raw loose enumeration
  ...
  the case a silently omitted leaf lands in; and a name `for-each-ref` reports
...
4. `test_evidence_ref_hazard_funnels_are_bounded_reaped_and_fail_closed`
   ...
   reaped child for that call. Each remaining sub-case must state its own
   injection point, and that point must be one this Plan's own ordering
   actually reaches, because raw loose enumeration runs first and rejects a
   non-regular leaf before any Git query is issued. Each sub-case asserts only
   what its site produces:
   ...
   2. **Exact-ref fallback.** To reach it, the strict snapshot must first
      succeed, so inject the FIFO after both complete namespace snapshots and
      before exact ref resolution. `for-each-ref` then exits zero and empty and
      falls through to `git symbolic-ref`, which is the call measured to
      block. Assert that funnel 1 expires the 2.0-second bound, terminates the
      child process group, kills after the 0.5-second grace, reaps
      synchronously, and fails `FOREIGN_REF`.
   3. **Create-only CAS.** To reach `update-ref --no-deref`, exact ref
      ...
      process-group termination, grace, kill, synchronous reap, and
      `FOREIGN_REF`.
   4. **Lock residue.** Continuing from sub-case 3, remove the FIFO — with it
      still in place the next run re-blocks at the same fallback and reaches
      no verdict, so the residue is only observable once the hazard is gone.
      A fresh run must then diagnose the surviving zero-byte `<ref>.lock` as
      `STALE_REF_LOCK` rather than `FOREIGN_REF`, exit 2 with stderr-only
      diagnostics naming the literal lock path, and terminate. Assert that the
...
| `STALE_REF_LOCK`              | a zero-byte `<canonical-leaf>.lock` sibling is observed at an admitted evidence-leaf path inside the fixed namespace; the gate stops here with exit 2 and mutates nothing, and makes no claim about which process left it         |
...
an observed zero-byte `<canonical-leaf>.lock` sibling inside the fixed
...
the sequence ends there, with exit 2 and stderr-only diagnostics. It is not a
...
residue, both of which stop the run before any ref can be written, every
consumed attempt, accepted or not, must end in a create-only evidence
ref. A
...
- Step 0e closes within its five-round breaker with the explicit POSIX bundle
  ...
  every whole-file read — fully reaped where the funnel spawns and closed on a
  named deadline where it only opens a descriptor — with a distinct terminal
  read-only stale-ref-lock state that clears nothing,

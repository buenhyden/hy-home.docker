...
| T-TSC-003 | Root content archive provenance migration | final_exception_remediated_re_review_pending |
...
| T-TSC-003 | Initial RED evidence is recorded below. Final-exception RED used payload-free unordered `- NeTsH …` and numeric ordered-list mutations; one method failed both subcases because the current first token was the list marker and no `stale-command-body` was emitted. | Final-exception GREEN passed listed/bare/current 3/3, `ArchiveProvenanceTests` 8/8, and full lifecycle 114/114. Exactly one Markdown list marker is normalized before exact token comparison; prose and `netsh_command` remain non-findings. Explicit-base metadata, immutable target/generated bytes, hashes/counts, compile, Ruff, and diff gates pass with all 483 review pairs pending. | final_exception_remediated_re_review_pending |
...
| T-TSC-003 | recorded | latest CHANGES REQUIRED C0/I1/M0; `9a161e18f7806e2268191d3b7d21fb10192859a2` | not_run | The sole Markdown-list I-01 is remediated under the user-approved third/final exception with focused RED/GREEN and lifecycle evidence. Final authorized specification re-review remains required; quality has not run and no manifest verdict is promoted. |
...
| T-TSC-003 final specification fix | `test(docs): detect listed archived commands` | current logical commit; identity intentionally not self-recorded | Listed-command RED one method/two subcases; focused GREEN 3/3; archive 8/8; lifecycle 114/114; explicit-base metadata, immutable bytes/invariants, compile, Ruff, and diff gates pass; final authorized re-review pending. |

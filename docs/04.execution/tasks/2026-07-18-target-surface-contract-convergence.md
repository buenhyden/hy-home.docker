...
| T-TSC-004 | Deprecated runtime and duplicate disposition | opensearch_spec_remediation_ready_seaweedfs_deferred |
...
| T-TSC-004 | Prior runtime/evidence RED/GREEN remains recorded above. OpenSearch duplicate RED: 1/1 failed because the `.example` existed. Initial specification review FAIL C0/I1/M0 reproduced on committed synthetic candidate `78bfff5d637d805ea2731e2a049a6299fa9516b1` because the regression queried the deleted blob through `HEAD:path`. | I-01 remediation pins both pre-delete blobs to immutable real commit `bad9a4a0aeb014c9eee398ea039ec0076723cd68` and checks only the retained empty blob at current `HEAD`; review-only objects are not test dependencies. Focus and target 19/19 GREEN; direct manifest emits exactly the sole held `manifest-target-missing`; diff hygiene and empty index pass. Fresh specification re-review remains pending. SeaweedFS deletion is deferred to Task 6 generated freshness. | opensearch_spec_remediation_ready_seaweedfs_deferred |
...
| T-TSC-004 | manifest promotion recorded | Evidence PASS C0/I0/M0; `f300b4f88cc6672445ac25a06602adb62381f7c0..d8be0bb043c97996bbbee73560dd8fa1f6729dea` | Evidence APPROVED C0/I0/M0; same range | Runtime package reviews also passed over the earlier synthetic range. Exact InfluxDB 2 row is pass/pass; all other 482 rows remain pending/pending. |

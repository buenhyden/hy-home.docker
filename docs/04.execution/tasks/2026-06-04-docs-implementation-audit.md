...
| 05.operations   | 271  | 199 active / 72 README-like missing status | open-notebook draft→active; policy/guide profile 중복 정리; bucket-root leaf 문서를 목적 폴더로 정리; `05-messaging`/`07-workflow` current-truth 정정 | 2 archived | misplaced ksqlDB guide와 duplicate Airflow DAG guide는 `docs/98.archive`; 나머지 orphan 없음(`failures=0`) |
...
- Stage 01-05 inventory: `docs/01.requirements=25`, `docs/02.architecture=51`, `docs/03.specs=43`, `docs/04.execution=125`, `docs/05.operations=271`, total `515`.

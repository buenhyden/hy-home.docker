...
guide, `07-workflow`의 duplicate Airflow DAG guide, `08-ai`의 duplicate Ollama setup/inference guides를
...
| 05.operations   | 269  | 197 active / 72 README-like missing status | open-notebook draft→active; policy/guide profile 중복 정리; bucket-root leaf 문서를 목적 폴더로 정리; `05-messaging`/`07-workflow`/`08-ai` current-truth 정정 | 4 archived | misplaced ksqlDB guide, duplicate Airflow DAG guide, duplicate AI setup/inference guides는 `docs/98.archive`; 나머지 orphan 없음(`failures=0`) |

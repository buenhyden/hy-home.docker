...
| DLR-008 | Infra/secrets spec inventory drift | Count-only revalidation reports 94 secret/cert files under `secrets/` after excluding Markdown registry/README documents, while the active infra/secrets spec still carried the older 76 count | Updated the current spec inventory count only; no secret file contents or values were read or changed | Closed |
...
| Infra/secrets count-only revalidation | PASS | `find secrets -type f ! -name '*.md' \| wc -l` reports 94 secret/cert files; root README already matched and the active spec inventory was updated without reading secret contents |

...
- **Full Test Commands**: the pre-remediation metadata module passed `102/102`;
  validation discovery passes `146/146`; explicit-base changed/new metadata
  selects `51` paths with `0` violations. The complete post-fix bundle remains
  intentionally deferred until the controller records the first clean commit.

...
| T-TSDC-004R-2 corrected identity-cleanup Plan | bounded future implementer | pending fresh reviewer | pending different fresh reviewer | unique design-checkpoint commit through one successor implementation commit | one attempt and one review pair required; Wave B blocked | The successor may begin only after the exact unique `docs(plan): record typed gate identity design checkpoint` commit and must satisfy the corrected Task 4.2S contract. |
...
| T-TSDC-004R-2 identity-cleanup Plan correction | Close scope-baseline, cleanup-priority, acquisition-reap, post-reap, and `ESRCH` findings | `docs(plan): close typed gate identity review findings` | this logical commit | Plan and Task static validation required before the uniquely named controller checkpoint; no implementation or gate execution is authorized by this correction alone. |
| T-TSDC-004R-2 identity-cleanup design checkpoint | Freeze the corrected Plan as the successor implementation scope baseline | `docs(plan): record typed gate identity design checkpoint` | successor evidence commit | The implementation oracle resolves this exact unique subject and requires precisely the two runtime files, two existing test files, and this Task ledger after the checkpoint. |

...
| T-TSDC-004 | Missing wrapper; four missing-module errors; bounded reader 2 failures/1 error; `148 != 141`; missing local wrapper-test route; missing purpose-folder registration; mixed quoted/unquoted `on` failed both orderings because no ambiguity error was raised. | Initial workflow 9/9 and remediation 11/11; fake-binary wrapper; focused CLI 7/23/8; exact manifest 148/89/59/0 and advisory; repaired script-usage section; mixed `on` is value-free fail-closed. | Actionlint, ShellCheck, Ruff, compile, Bash, and diff gates pass. Governance routing is 33/34 with only missing predecessor dependency `html5lib`; aggregate Task 4 sections pass while separately listed predecessor/generated-evidence groups remain. | re-review pending |
...
| T-TSDC-004 | Task 4 workflow/QA implementation agent | pending fresh reviewer | one Important; remediation implemented; re-review pending | `6fabbf05..faaa2eb2` | re-review pending | Mixed quoted/unquoted top-level `on` could hide a forbidden trigger. Both orderings now fail closed with a value-free ambiguity error; all 148 manifest verdict pairs remain pending and no remote or real pre-commit action ran. |
...
| T-TSDC-004 quality remediation | Reject ambiguous mixed workflow trigger keys | `fix(ci): reject ambiguous workflow triggers` | pending local commit | mixed-key RED 2/2; focused GREEN 11/11; re-review pending |

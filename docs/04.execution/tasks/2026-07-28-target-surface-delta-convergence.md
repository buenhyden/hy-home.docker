...
| T-TSDC-004R-2 identity-cleanup design return | bounded future implementer | pending fresh reviewer | pending different fresh reviewer | this design commit through one successor implementation commit | one attempt and one review pair required; Wave B blocked | The successor must preserve all four `17bb5cdd` freezes and exact 94-test discovery while closing capability cleanup and pidfd identity finalization. |

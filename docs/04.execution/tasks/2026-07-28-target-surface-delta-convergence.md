...
| T-TSDC-003 | Reconcile static versions and verified active lifecycle drift | infra-support/docs | TSDC-003, 008–009 | version, hardening, supply-chain checks | Task 3 static-version implementation agent | remediated; re-review pending |
...
| T-TSDC-003 | Registry contract emitted six subtest failures; the bounded suite then emitted those six plus one Keycloak literal failure. The hidden Dozzle follow-up and stale 136-row oracle each failed one exact regression. First review remediation produced 17 exact resolver/static failures; final quoted-service remediation produced six focused failures. | Initial focused version/lifecycle tests passed 7/7 and the repository manifest oracle passed 1/1. First review-remediation tests passed 11/11; final affected methods pass 4/4 and the full focused module passes 16/16 with quoted target, sibling-boundary, theft, and duplicate-key coverage. | Sync/provenance, 11-tier hardening, supply-chain 13 fixtures, successor advisory, repository static oracle, Ruff, compile, Bash, scoped ShellCheck, and diff gates pass. All reported findings are implemented; independent re-reviews remain pending. | remediated; re-review pending |
...
- Both Important findings are implemented, but independent re-reviews remain
  pending. No successor manifest verdict was promoted.
...
- Final scoped ShellCheck passes the hardening script and the repository
  ...
  lines 3546 and 3555; they remain outside this remediation. Independent
...
| T-TSDC-003 | Task 3 static-version implementation agent / replacement remediation implementers | C0/I1/M0; stale-absence finding remediated, re-review pending | C0/I1/M0 initial plus C0/I1/M1 remediation review; all findings implemented, re-review pending | `b1e62873..c04946e5`; final remediation commit pending | remediated; re-reviews pending | Exact stale-absence protects all 14 mappings. One canonical quoted/unquoted service-key grammar now owns target uniqueness and sibling boundaries; missing images cannot consume sibling values. The Dozzle diagnostic is current. All 140 manifest verdict pairs remain pending. |
...
| T-TSDC-003 final remediation | Close quoted service resolver boundary and currentize Dozzle diagnostic | `fix(infra): close quoted service resolver gap` | pending current remediation commit | quoted-key RED six failures; affected GREEN 4/4 and full focused 16/16; all hardening tiers, sync/provenance, repository static oracle, Bash, scoped ShellCheck, Ruff, compile, and diff hygiene pass; independent re-review pending |

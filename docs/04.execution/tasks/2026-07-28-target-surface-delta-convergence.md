...
| T-TSDC-004R-2 phase-owned runner design return | Controller / bounded future implementer | pending fresh review | pending fresh review | starts after `483d3a47` | corrected checkpoint pending; Wave B blocked | Task 4.2U freezes the accepted adapter pair and permits only runner, runner-test, and Task-ledger changes for one outer phase-owning controller with transition-specific witnesses. |
...
| T-TSDC-004R-2 phase-owned runner Plan correction | Close bound-process lifecycle transition gaps | `docs(plan): define phase-owned runner recovery` | in progress | Must freeze the accepted adapter pair, restrict implementation to three paths, and define one outer controller plus transition-specific RED witnesses. |
| T-TSDC-004R-2 phase-owned runner checkpoint | Freeze the corrected runner-only successor scope | `docs(plan): record phase-owned runner checkpoint` | pending | The implementation oracle requires one matching commit after the corrected Plan and supersedes no accepted adapter behavior. |

...
| T-TSDC-004R-2 design reset | Controller / bounded future implementer | pending new independent reviewer | pending different new reviewer | starts after `docs(plan): redesign typed gate runner cleanup` | one redesigned implementation attempt only | The runner must own descriptor capability and one process group; adapters duplicate validated inherited FDs with CLOEXEC and create no nested sessions. RED/GREEN must prove registered descriptor-root gates, no orphan descendants, and bounded HOME cleanup. |
...
| T-TSDC-004R-2 design reset | Finalize descriptor capability and runner-owned invocation lifecycle | `docs(plan): redesign typed gate runner cleanup` | pending | No production/test change starts until this docs-only reset is committed. One bounded attempt must prove descriptor-root setup.compose-env/zizmor integration, root replacement/sitecustomize rejection, child/grandchild cleanup across timeout/overflow/read-error/nonzero, and bounded HOME retry; the five-module accounting is 94 total = 83 pass + 11 Wave-C skips. |
...
| T-TSDC-004R-2 typed gate runner | returned to design; redesigned remediation pending | Initial `5819c8ab` reviews were Spec C0/I2/M0 and Quality/Security C0/I5/M1. Fresh reviews of remediation `5819c8ab..17bb5cdd` were Spec C0/I2/M0 and Quality/Security C0/I2/M1; neither authorizes Wave B. Manifest verdicts remain pending. | commit the docs-only Task 4.2R redesign, execute at most one bounded lifecycle remediation attempt, obtain a new independent C0/I0 review pair, then commit controller review evidence before Wave B |
...
approved. Task 4.2 is returned to design after its first remediation reviews;
Wave B and Tasks 5–6 remain blocked until the redesigned one-attempt Task 4.2R
subtask receives a new C0/I0 review pair and controller evidence commit. The

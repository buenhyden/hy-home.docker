...
| 2026-07-30 | T-TSDC-004R-2 recovery-owned finalization design return | Controller / bounded future implementer | Task 4.2V moves normal and error finalization under one actual `finally`, nests KILL/readiness/conditional-reap/close ownership so every interruption reaches later state-permitted cleanup, makes the lifecycle object the sole bound-process identity, and fixes the pre-bind ordinary-exception taxonomy. The successor preserves the same three-path implementation boundary, adapter pair, four freezes, discovery/projection invariants, and one-attempt review gate. The actual correction SHA is recorded by the succeeding checkpoint commit. |
...
| T-TSDC-004R-2 recovery-owned finalization design | Controller / bounded future implementer | pending fresh review | pending fresh review | starts after `ad2df527` | corrected checkpoint pending; Wave B blocked | Task 4.2V preserves the three-path allowlist and requires one lifecycle identity, nested cleanup finalizers, fixed pre-bind taxonomy, and recovery-transition RED witnesses. |
...
| Whole branch | not applicable | pending fresh reviewer | pending different fresh reviewer | not available | pending | Task 4.2U is review-rejected and exhausted. Wave B remains blocked until the recovery-owned Task 4.2V checkpoint receives fresh Plan approval, its sole runner-only implementation receives fresh specification and quality/security `C0/I0` reviews, and controller-owned review evidence is committed. |
...
| T-TSDC-004R-2 recovery-owned finalization Plan correction | Own cleanup transitions and pre-bind taxonomy | `docs(plan): define recovery-owned runner finalization` | recorded by the succeeding checkpoint | Requires one lifecycle process identity, actual outer and nested finalizers, after-completed-recovery-transition witnesses, fixed value-free `Popen` ordinary-exception handling, the same three-path allowlist, and all prior freezes. |
...
| T-TSDC-004R-2 typed gate runner | returned to recovery-owned design | The sole Task 4.2U attempt passed local gates but failed fresh quality/security review on recovery-transition ownership and pre-bind ordinary-exception normalization. The accepted adapter pair remains frozen, all manifest verdicts remain pending, and Wave B has no authority. | approve the runner-only Task 4.2V checkpoint, complete its sole bounded implementation attempt, obtain a fresh implementation C0/I0 pair, then commit controller-owned evidence |
...
design return. Wave B and Tasks 5–6 remain blocked until its corrected
checkpoint receives a fresh Plan C0/I0 pair, its sole implementation receives
a fresh implementation C0/I0 pair, and controller-owned review evidence is
committed.

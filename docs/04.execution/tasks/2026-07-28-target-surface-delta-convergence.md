...
| T-TSDC-004R-2 executable identity-cleanup Plan | bounded future implementer | pending fresh reviewer | pending different fresh reviewer | unique executable-design checkpoint through one successor implementation commit | one attempt and one review pair required; Wave B blocked | The successor may begin only after the exact unique `docs(plan): record executable typed gate identity checkpoint` commit and must satisfy both executable scope oracles. |

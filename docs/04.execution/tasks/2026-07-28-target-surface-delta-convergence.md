...
| T-TSDC-004R-2 typed-interruption design return | Controller / bounded future implementer | pending fresh review | pending fresh review | starts after `17645f2a` | design correction in progress; Wave B blocked | Successor Plan Task 4.2T preserves the same five-path implementation boundary and four `17bb5cdd` freezes while requiring adapter ordinary-exception normalization, BaseException-safe runner cleanup, conditional-reap wording, and explicit metadata/traceability evidence. |
...
| T-TSDC-004R-2 typed-interruption design correction | Define ordinary-exception normalization, BaseException cleanup, and evidence wording | `docs(plan): define typed interruption recovery` | in progress | Must record the failed 4.2S review pair, establish a new checkpoint subject, and preserve exact five-path scope before another implementation attempt. |

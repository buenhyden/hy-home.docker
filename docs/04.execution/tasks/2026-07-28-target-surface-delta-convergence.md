...
| 2026-07-30 | T-TSDC-004R-2 phase-owned runner gate reconciliation | Controller | The Plan and Task ledger now require a new reconciled Task 4.2U checkpoint, its sole runner-only implementation attempt, fresh implementation `C0/I0` reviews, and controller-owned evidence before Wave B. The actual correction SHA is recorded by the succeeding reconciled checkpoint commit. |
...
| T-TSDC-004R-2 reconciled phase-owned runner design | Controller / bounded future implementer | pending fresh review | pending fresh review | starts after `483d3a47` | reconciled checkpoint pending; Wave B blocked | Task 4.2U freezes the accepted adapter pair and permits only runner, runner-test, and Task-ledger changes for one outer phase-owning controller with transition-specific witnesses. |
...
| Whole branch | not applicable | pending fresh reviewer | pending different fresh reviewer | not available | pending | Wave B remains blocked until the reconciled Task 4.2U checkpoint receives fresh Plan approval, its sole runner-only implementation attempt receives fresh specification and quality/security `C0/I0` reviews, and controller-owned review evidence is committed. |
...
| T-TSDC-004R-2 phase-owned runner gate reconciliation | Align the branch gate with Task 4.2U | `docs(plan): reconcile phase-owned runner gate` | recorded by the succeeding checkpoint | Replaces the stale Task 4.2T prerequisite while preserving the runner-only allowlist, accepted adapter freeze, lifecycle design, and Wave B block. |
...
| T-TSDC-004R-2 typed gate runner | returned to phase-owned design | The 4.2S and 4.2T attempts failed fresh quality/security review. The accepted adapter pair remains frozen, all manifest verdicts remain pending, and Wave B has no authority. | approve the runner-only Task 4.2U checkpoint, complete its one bounded implementation attempt, obtain a fresh C0/I0 pair, then commit controller-owned evidence |
...
return that freezes the accepted adapter pair. Wave B and Tasks 5–6 remain
blocked until its corrected checkpoint receives a fresh Plan C0/I0 pair, its
one implementation receives a fresh implementation C0/I0 pair, and
controller-owned review evidence is committed. The original five-round

...
| T-TSDC-003 | Registry contract emitted six subtest failures; the bounded suite then emitted those six plus one Keycloak literal failure. The hidden Dozzle follow-up and stale 136-row oracle each failed one exact regression. Review remediation then produced 17 exact resolver/static failures before implementation. | Initial focused version/lifecycle tests passed 7/7 and the repository manifest oracle passed 1/1. Review-remediation focused tests pass 11/11 with exact stale absence, safe resolver scalar, ambiguity, payload, no-follow, regular-file, and size coverage. | Sync/provenance, 11-tier hardening, supply-chain 13 fixtures, successor advisory, Ruff, compile, Bash, ShellCheck, and diff gates pass. Both initial Important findings are remediated; independent re-reviews remain pending. | remediated; re-review pending |
...
| T-TSDC-003 | Task 3 static-version implementation agent / replacement remediation implementer | C0/I1/M0; stale-absence finding remediated, re-review pending | C0/I1/M0; fail-open resolver finding remediated, re-review pending | `b1e62873..5460e5fc`; remediation commit pending | remediated; re-reviews pending | Exact stale-absence and old-plus-new negative coverage now protects all 14 mappings. The resolver is bounded, no-follow, duplicate-safe, value-free on rejection, and exact for Keycloak/Dozzle. All 140 manifest verdict pairs remain pending. |
...
| T-TSDC-003 remediation | Harden static version assertions and Compose image resolution | `fix(infra): harden static version assertions` | pending current remediation commit | resolver/static RED 17 failures; final focused 11/11; sync/provenance, all hardening, supply-chain 13, delta advisory, Ruff, compile, Bash, ShellCheck, and diff gates pass; independent re-reviews pending |

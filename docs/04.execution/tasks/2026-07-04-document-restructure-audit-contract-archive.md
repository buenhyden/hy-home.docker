...
- [ ] T-DRA-003 Update Stage 99 support contracts and minimal Stage 00 rules if needed.
...
| --- | --- | --- |
| Template-token scan | Focused `rg` scan for unresolved template tokens in the new task evidence and task README. | PASS: no matches. |
| Whitespace | `git diff --cached --check` | PASS: no whitespace errors. |

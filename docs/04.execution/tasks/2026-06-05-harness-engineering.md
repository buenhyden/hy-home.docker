...
- Remote GitHub required-check status is not verified by this local task.
- `validate-docker-compose.sh --preflight` is intentionally not part of the
  default wrapper because it checks local operational readiness.

## Follow-up Tasks

- None for the implemented wrapper. Remote GitHub required-check status remains
  PR-time evidence.

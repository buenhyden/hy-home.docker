...
The sibling
[Task ledger](tasks/tsk-0001-delta-convergence.md)
...
- Modify direct-impact operations documents:
  ...
  `docs/05.operations/catalog/06-observability/0045-prometheus/policy.md`, and
...
git diff --exit-code 17bb5cdd -- docs/90.references/data/0073-target-surface-delta-manifest/data.yaml docs/90.references/data/0074-target-surface-delta-summary/README.md
python3 scripts/validation/check-target-surface-delta-contract.py --mode advisory
...
python3 scripts/validation/check-document-metadata.py \
  --mode report \
  --output docs/90.references/audits/0023-frontmatter-semantic-inventory/README.md
```
...
if rg -n '^  (spec_verdict|quality_verdict): (pass|fail)$' \
  docs/90.references/data/0073-target-surface-delta-manifest/data.yaml; then
  exit 1
fi
```

...
git show 57259e24:"$LEAF" | grep '^#' | sed 's/^#* //' | while IFS= read -r h; do
  grep -rlF "$h" docs --include='*.md' \
    ...
    | grep -v 'spec-0152-deleted-reference-leaf-disposition' \
    | grep -q . || printf '%s\n' "$h"
done | wc -l                                    # 10
```
...
- [SDLC taxonomy convergence](../spec-0136-sdlc-taxonomy-convergence/spec.md)
- [Agentic research pack rebuild Task](../../04.execution/tasks/2026-08-08-agentic-research-pack-rebuild.md)

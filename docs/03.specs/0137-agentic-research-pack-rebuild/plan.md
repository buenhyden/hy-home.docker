...
5. Run `python3 scripts/validation/check-document-links.py --mode traceability`
   ...
   exits and summaries. At this exception's entry, traceability is FAIL/exit 1
   with the one inherited Task 0001 `document-not-regular`; alignment is
   FAIL/exit 1 with 42 inherited findings, `documents=359 links=2472`. Require zero introduced
...
- Canonical destination:
  `docs/90.references/research/0002-agentic-engineering-research-pack/`.
- Closed scope axis: `agentic`, `architecture`, `common`, `docs`, `infra`,
  `ops`, `qa`, and `security`.
- The destination has exactly `README.md` plus the twenty leaves named in the
  Spec; no register, handoff, or compatibility file is added.
- Stage 90 is advisory. It cannot create policy, runtime, remote, security,
  provider-entitlement, or model-availability truth.
- Every claim distinguishes upstream capability, tracked local adoption, and
  authorized observed execution. Missing proof is `UNVERIFIED`.
- `RES-0002/README.md` selects the Stage 99 research profile with artifact ID
  `RES-0002`. Preserve the independently established Task 9 profile and identity
  assignments for leaves; do not allocate substitute identities.
...
- Do not access or mutate containers, runtime services, remote settings,
  credentials, secrets, private provider state, or user-global configuration.
- Graphify `f8a72211` is stale and advisory. Corroborate graph leads against
  tracked source, Stage 00 governance, Stage 99, and current stage documents.
- Use a fresh implementer per logical content unit and fresh independent
  rules/specification and documentation-quality reviewers. A unit advances only
  at C0/I0/M0. Path-scoped validators must exit zero. Repository- or
  corpus-wide validators record their raw status; a non-final logical unit may
  advance only with zero attributable findings, while inherited findings remain
...
Reset the stale dated-pack execution authority, bind all prospective work to
Task 0004, wait for the independently owned structural dependency, and then
author a current research pack aligned with workspace-governance simplification.
...
No unit owns the parent Stage 90 README, a generator, a dated pack, SPEC-0153,
or any runtime/provider configuration.

### Stable leaf identity prefixes
...
| Leaf | Prefix |
| --- | --- |
| `workspace-baseline.md` | `WB` |
| `scope-application-matrix.md` | `SAM` |
| `harness-engineering.md` | `HE` |
| `loop-engineering.md` | `LE` |
| `provider-implementation-comparison.md` | `PIC` |
| `agent-instructions-vibe-coding.md` | `AIV` |
| `provider-model-landscape.md` | `PML` |
| `agent-model-selection.md` | `AMS` |
| `ai-agent-catalogs.md` | `AAC` |
| `memory-hierarchy.md` | `MH` |
| `spec-driven-sdlc.md` | `SSD` |
| `sdlc-document-roles.md` | `SDR` |
| `document-metadata-lifecycle.md` | `DML` |
| `documentation-architecture.md` | `DA` |
| `llm-wiki-system.md` | `LWS` |
| `automation-pipeline-workflow.md` | `APW` |
| `quality-ci-formatting.md` | `QCF` |
| `verification-validation.md` | `VV` |
| `docker-compose-infrastructure.md` | `DCI` |
| `security-governance.md` | `SG` |
...
| --- | --- |
| Harness/providers | OpenAI Codex source `https://github.com/openai/codex/blob/main/codex-rs/hooks/src/lib.rs`; SessionEnd implementation `https://github.com/openai/codex/blob/main/codex-rs/hooks/src/events/session_end.rs`; Codex configuration `https://developers.openai.com/codex/config-reference/`; Claude hooks `https://code.claude.com/docs/en/hooks`; Claude subagents `https://code.claude.com/docs/en/sub-agents`. |
| Models | OpenAI models `https://developers.openai.com/api/docs/models`; Anthropic models `https://platform.claude.com/docs/en/about-claude/models/overview`; Anthropic selection guidance `https://platform.claude.com/docs/en/about-claude/models/choosing-a-model`. Documentation is not entitlement or runtime proof. |
| Agent catalogs and memory | `agency-agents` at `https://github.com/msitarzewski/agency-agents/tree/ebe9c99acb5c96f9468de368d8bead775387d1a7` with MIT license; ReAct `https://arxiv.org/abs/2210.03629`; Reflexion `https://arxiv.org/abs/2303.11366`; MemGPT `https://arxiv.org/abs/2310.08560`. |
| SDLC/docs | ISO/IEC/IEEE 12207:2026 `https://www.iso.org/standard/90219.html`; ISO/IEC/IEEE 29148:2018 `https://www.iso.org/standard/72089.html` with its current “to be revised” status captured; ISO/IEC/IEEE 42010 Architecture Description `https://www.iso.org/standard/74393.html`; ISO/IEC/IEEE 15289 `https://www.iso.org/standard/74909.html`; Spec Kit `https://github.com/github/spec-kit`; Diataxis `https://diataxis.fr/`; NIST SP 800-61r3 `https://csrc.nist.gov/pubs/sp/800/61/r3/final`; Google SRE postmortem guidance `https://sre.google/sre-book/postmortem-culture/`. |
...
The comparison core is the first four fields. Detail is trimmed and internal
whitespace is collapsed; elapsed time, ordering, summary counters, and other
non-finding output never enter the identity set. Raw exit status and summary are
still preserved separately. A later exact five-field match is inherited. A new
core is attributable. A matching core with changed detail is attributable as a
changed/worsened finding and the prior detail is retained for comparison. A
...
1. Complete the owned content and publish only already observed validator and
   ...
   not yet performed is recorded as `Not Run`, never prospectively as PASS.
2. Rerun the unit's exact path-scoped validators, repository/corpus comparison,
   and diff checks on the final uncommitted tree.
3. Fresh independent reviewers inspect that exact final diff/range. Their
   terminal C0/I0/M0 verdict stays outside the reviewed tree in the execution
   handoff/commit evidence. No file mutation follows a terminal review before
   commit.
4. Commit the exact reviewed tree only after all required terminal reviewers
   return C0/I0/M0. Any edit or nonzero finding invalidates the terminal review
   and restarts publication, validation, and terminal review. The handoff
   records the external verdict and resulting commit ID without a self-
   recording Task commit.
...
```bash
git add docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(task): bind canonical research baseline"
```

Expected: focused metadata and diff check pass, fresh dual review is C0/I0/M0,
and only Task 0004 is in the evidence commit.

### Task 3: Author Foundation and Aggregate Contracts
...
find docs/90.references/research/0002-agentic-engineering-research-pack \
...
Use tracked files and these commands; record commit and commands in
`workspace-baseline.md`:

```bash
git rev-parse HEAD
git ls-files docs/00.agent-governance docs/01.requirements docs/02.architecture \
  docs/03.specs docs/05.operations docs/90.references docs/99.templates \
  .claude .codex .agents .github scripts tests
rg -n 'SessionEnd|SubagentStop|profile_id: research|run-ci-gate.py|public_suites' \
  docs/00.agent-governance docs/99.templates .claude .codex scripts tests
```
...
- Modify: `harness-engineering.md`
- Modify: `loop-engineering.md`
- Modify: `provider-implementation-comparison.md`
- Modify: `agent-instructions-vibe-coding.md`
- Modify: Task 0004

All research paths in Tasks 4–8 are relative to
`docs/90.references/research/0002-agentic-engineering-research-pack/`.

**Interfaces:**

- Consumes: foundation schemas and the harness/provider primary-source baseline.
- Produces: harness element model, loop state/stop/escalation model,
  provider-neutral-to-native mapping, and instruction precedence/application.

- [ ] **Step 1: Author from the preserved provider snapshot**

Use the official Claude hooks/subagent pages and OpenAI Codex hook source as
observed during the preserved 2026-08-23 read-only research. The snapshot
finding is that Codex includes upstream `SessionEnd` and `SubagentStop`; this is
a capability finding only. Preserve the captured platform or version
limitations as caveats rather than inferring runtime success. Do not re-access
the external pages in this unit.

- [ ] **Step 2: Remeasure local adoption after Task 9**

```bash
rg -n 'SessionEnd|SubagentStop|SubagentStart|PreToolUse|PostToolUse' \
  .claude .codex docs/00.agent-governance scripts tests
rg -n '^\[agents\.|hooks|multi_agent|model|reasoning' \
  .codex docs/00.agent-governance .agents
```

Expected: tracked matches become local-adoption evidence only. Absence or an
unexecuted configuration remains `UNVERIFIED` for runtime behavior.

- [ ] **Step 3: Author the four leaves**

Cover harness inputs/context/tools/policies/sandbox/observability/evals and
recovery; loop perceive-plan-act-observe-evaluate-stop/escalate controls;
Claude/Codex common construction and irreducible differences; and provider
instruction precedence, durable context, skills, subagents, hooks, and
vibe-coding risk controls. Map every claim to all eight scopes.

- [ ] **Step 4: Reconcile README rows and Task evidence**
...
git add docs/90.references/research/0002-agentic-engineering-research-pack/README.md \
  ...
  docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(research): document harness and provider loops"
```
...
- Modify: `provider-model-landscape.md`
- Modify: `agent-model-selection.md`
- Modify: `ai-agent-catalogs.md`
- Modify: `memory-hierarchy.md`
- Modify: README and Task 0004

**Interfaces:**

- Consumes: foundation schema and official model/catalog/paper baseline.
- Produces: mutable provider landscape, work-characteristic selection matrix,
  catalog adoption analysis, and short/long/domain memory lifecycle.

- [ ] **Step 1: Use the preserved provider-document observations**

Author from the OpenAI and Anthropic model families, model IDs,
reasoning/effort controls, context/output constraints, and stated selection
guidance captured in the 2026-08-23 snapshot. Mark docs availability,
configured selection, entitlement, API acceptance, and successful execution as
five separate evidence states. Do not refresh mutable model pages during
execution.

- [ ] **Step 2: Pin catalog and memory evidence**

Use the preserved 2026-08-23 observation of `agency-agents` commit
`ebe9c99acb5c96f9468de368d8bead775387d1a7` and its MIT license, and analyze
structure/rules/implementation without importing its personas as local
authority. Use the preserved ReAct, Reflexion, and MemGPT observations as
primary conceptual sources.

- [ ] **Step 3: Remeasure local model and memory surfaces**

```bash
rg -n 'model|reasoning|effort|memory|retention|current.md' \
  docs/00.agent-governance .claude .codex .agents
```
...
git add docs/90.references/research/0002-agentic-engineering-research-pack/README.md \
  ...
  docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(research): document models agents and memory"
```
...
- Modify: `spec-driven-sdlc.md`
- Modify: `sdlc-document-roles.md`
...
Record the 2026-08-23 observations of ISO/IEC/IEEE 12207:2026 without inventing
...
```bash
rg -n 'profile_id: (spec|plan|task|guide|incident|postmortem|policy|runbook|research)' \
  docs/03.specs docs/05.operations docs/90.references docs/99.templates
rg -n 'Stage 04|docs/04.execution|ops-[0-9]|release.template|profile_id: release' \
  docs/00.agent-governance docs/03.specs docs/05.operations docs/90.references docs/99.templates
```
...
purpose, trigger, owner, consumer, system, evidence, rules, and relationships.
...
- Modify: `automation-pipeline-workflow.md`
- Modify: `quality-ci-formatting.md`
- Modify: `verification-validation.md`
- Modify: README and Task 0004

**Interfaces:**

- Consumes: GitHub Actions, NIST SSDF, SLSA v1.2, IEEE 1012-2024, and
  ISO/IEC/IEEE 29119 evidence plus tracked local workflows and validators.
- Produces: pipeline evidence ladder, QA gate taxonomy, and independent V&V
  responsibility model.

- [ ] **Step 1: Analyze secure workflow and provenance controls**

Cover least-privilege `permissions`, full-length action SHA pinning,
environments/approvals, protected secrets, artifact attestations, dependency and
provenance controls, and local/PR/push separation. A tracked workflow proves
configuration only, not remote branch protection, enforcement, or a successful
run.

- [ ] **Step 2: Remeasure tracked QA and CI**

```bash
git ls-files .github .pre-commit-config.yaml scripts/validation tests
rg -n 'permissions:|uses:|run-ci-gate|format|lint|syntax|test|attest' \
  .github .pre-commit-config.yaml scripts/validation tests
```

Expected: local definitions and tests only; remote settings and run history are
not queried.

- [ ] **Step 3: Author the three leaves**

Distinguish verification (“built right” against specified requirements) from
validation (“right system” for intended use), name independence and lifecycle
...
python3 scripts/validation/check-document-metadata.py --mode check-changed \
  ...
  --changed-path docs/90.references/research/0002-agentic-engineering-research-pack/verification-validation.md \
  --changed-path docs/90.references/research/0002-agentic-engineering-research-pack/README.md \
  --changed-path docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
python3 scripts/validation/check-document-links.py --mode traceability
git diff --check
...
git add docs/90.references/research/0002-agentic-engineering-research-pack/README.md \
  ...
  docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(research): document delivery quality and validation"
```
...
- Modify: `docker-compose-infrastructure.md`
- Modify: `security-governance.md`
- Modify: README and Task 0004

**Interfaces:**

- Consumes: Docker/Compose, OCI, NIST, SLSA, Sigstore, OpenSSF, and OWASP
  sources plus tracked repository infrastructure/security definitions.
- Produces: declarative Compose/infrastructure evidence model and layered secure
  SDLC/supply-chain control analysis.

- [ ] **Step 1: Apply fixed pins and the preserved external snapshot**

Use compose-spec commit
`bd6ccc6581199b0103837b7f1529f1ea875d7362`, SLSA v1.2, and OpenSSF Scorecard
commit `d1fab88f54636ff366076edfc5c239f97b3c8e66`; combine them with the Docker,
OCI Image Spec, NIST SSDF/CSF 2.0, Sigstore, and OWASP observations
preserved from 2026-08-23. Do not re-access those mutable sources during this
unit.

- [ ] **Step 2: Remeasure tracked workspace definitions without runtime access**

```bash
git ls-files '*compose*.yml' '*compose*.yaml' infra scripts/validation .github
rg -n 'healthcheck|depends_on|profiles|secrets|read_only|cap_drop|attest|scorecard|sigstore|slsa' \
  --glob '*.yml' --glob '*.yaml' --glob '*.md' --glob '*.py' --glob '*.sh' \
  infra .github scripts docs/00.agent-governance
```
...
git add docs/90.references/research/0002-agentic-engineering-research-pack/README.md \
  ...
  docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(research): document compose infrastructure security"
```
...
- Consumes: all six reviewed content commits.
- Produces: one closed and internally consistent pack whose README aggregates
  reconcile with every leaf row and whose links stay within approved ownership.
...
```bash
rg -n 'docs/04.execution|Stage 04|ops-[0-9]|profile_id: release|release.template' \
  docs/90.references/research/0002-agentic-engineering-research-pack
rg -n 'guaranteed|always enforced|runtime verified|remote verified|model available' \
  docs/90.references/research/0002-agentic-engineering-research-pack
```

Expected: no obsolete authority. Any natural-language match is reviewed and
must carry evidence/caveat rather than an unsupported guarantee.
...
After fresh rules/specification and documentation-quality C0/I0/M0 review:

```bash
git add docs/90.references/research/0002-agentic-engineering-research-pack \
  docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(research): reconcile canonical research pack"
```

Expected: only actual reconciliation and Task evidence are committed.

### Task 10: Verify, Review, Integrate, and Finish
...
Record the accepted/frozen main commit as a literal ID, the pre-readiness branch
HEAD, exact ranges, commands, exit codes, counts, pre-publication reviews,
inherited debt, skips, rollback, and readiness for main integration. Keep
`status: active`; branch-local evidence cannot claim the future merge result.
Record the terminal publication review as `Not Run` inside Task 0004.
...
Fresh rules/specification and documentation-quality reviewers now inspect the
exact final Task diff and complete accepted-main-to-final-tree range. Their
terminal C0/I0/M0 verdict remains in the external execution handoff. Do not
mutate any file after this review. Only then run:

```bash
git add docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(task): record canonical research readiness"
git rev-parse HEAD
```

Capture the resulting literal commit ID as `readiness_head_commit` in the
external Task 0004 execution handoff. The tracked Task cannot contain the hash
of its own commit without changing that hash; no self-recording follow-up is
created.

- [ ] **Step 5: Run the finishing pre-integration checkpoint**
...
owner resolves it or separate approval changes the execution boundary. Do not
rewrite history.

- [ ] **Step 7: Publish post-merge completion evidence on main**
...
```bash
git add docs/03.specs/0137-agentic-research-pack-rebuild/tasks/tsk-0004-canonical-research-refresh.md
git commit -m "docs(task): complete canonical research integration"
git status --short --branch
```

Expected: focused metadata and diff check exit zero; the post-merge evidence
commit is on `main`; Task 0004 is completed; the main worktree is clean.
...
Use `superpowers:finishing-a-development-branch`. After confirming the research
merge commit is contained in main, the terminal Step 8 ladder is green, and the
main worktree is clean, remove only the research worktree and delete only its
merged local branch:

```bash
git worktree remove /home/hy/projects/hy-home.docker/.worktrees/agentic-research-canonical-refresh
git branch -d codex/0137-agentic-research-refresh
git worktree list
git status --short --branch
```

Expected: research worktree and branch are gone; Task 9 and legacy delta
worktrees remain listed; `main` is clean. No push or remote deletion occurs.
...
| Runtime or remote evidence required | A claim cannot be supported without container/provider/remote/secret access | Mark `UNVERIFIED` and defer. This Plan grants no such access. |
| Main merged-tree or terminal final-HEAD failure | Any applicable command is nonzero after merge or after the Task completion commit | Block cleanup and retain raw FAIL. Before cleanup, correct a premature completed Task through a separately authorized/reviewed lifecycle-correction commit or approved revert. Never reset, rewrite, or create an evidence-only self-recording loop. |

Every logical content commit is independently revertible. The default recovery
is a separately approved `git revert <commit>` followed by the same focused and
full available gates. Structural Task 9 commits are never reverted by this
research Plan.
...
1. Exact owned path census and `git status --short`.
2. Path-scoped `check-document-metadata.py --mode check-changed` with literal
   paths; it must exit zero.
3. Corpus-wide `check-document-links.py --mode traceability` for every content
   unit; record raw status, require zero attributable findings, and retain every
   inherited finding as FAIL/non-PASS.
4. Publish actual unit evidence into Task 0004, then rerun `git diff --check`
   and exact owned-name review on the final uncommitted tree.
5. Fresh terminal rules/specification and documentation-quality review of that
   exact tree, C0/I0/M0; keep the verdict external and make no later file edit.
6. One logical Conventional Commit of the exact reviewed tree; record its ID in
   the external handoff or a later independently reviewed Task update, never in
   the commit itself.
...
5. Invoke `superpowers:finishing-a-development-branch`; require every applicable
   full-ladder command to exit zero on the exact readiness commit before merge.
6. Post-merge rerun on `main`, then Task 0004 completion evidence committed on
   `main`.
7. Terminal full-ladder rerun on that completion commit; every applicable
   command exits zero before cleanup.

### Evidence rules

- Record commands and observed exit status; never write prospective PASS.
- A review result written into Task 0004 is necessarily pre-publication
  evidence. The terminal review of the resulting final tree remains external;
...
- Link findings are attributable when introduced by the research range and
  ...
  remains FAIL/non-PASS. It blocks main merge at the readiness-HEAD checkpoint
...
- After the branch-readiness commit, invoke
  ...
  merge occurs unless branch/base/clean checks and every applicable full-ladder
...
- The model-page observations are mutable inventory captured on 2026-08-23.
  Documentation availability is distinct from configuration, entitlement,
  API/runtime acceptance, and successful use.
- The local SDLC mapping uses Architecture Description and may explain `ARD` as

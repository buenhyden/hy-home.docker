...
`git add -A` staged with the evidence before the staged list was read back. It
was unstaged rather than committed: a generated graph snapshot is regenerated by
its own CLI and belongs to its own unit, never to a governance commit. Reading
`git diff --cached --numstat` before every commit is what caught it, and that is

...
what owns what. Libraries and command entrypoints share one directory, a
hand-maintained constant substitutes for the boundary the directory should
express, and nineteen of forty-five test modules are not reachable from any
registered gate. Eight of those nineteen are failing right now, measured, and no
one knew because nothing runs them.
...
- `scripts/manifest.yaml`, 80 registered entries.
...
third state. The measured cost of the third state is eight failing modules,
including one whose thirty-three tests all fail because their fixture copies a
document from a Spec Package that was deleted.
...
| A repaired test is made to pass rather than fixed | Each of the eight failures gets a stated root cause and a disposition |
...
1. Every file under `scripts/lib/` is importable and defines no CLI entrypoint.

...
class MigrationStateTests(unittest.TestCase):
    ...
    def read(self, document):
        ...
        with (
            ...
            mock.patch.object(
                ...
                return_value=copy.deepcopy(self.approved),
                create=True,
            ),
        ):
    ...
    def compact(self):
        ...
        additions.extend(
            {
                "source_path": f"docs/03.specs/{slug}/spec.md",
                "target_path": None,
                "artifact_id": f"SPEC-{slug[:4]}",
                "action": "delete",
                "recovery_commit": self.commit,
            }
            for slug in (
                "0090-workspace-audit-2026-05",
                "0091-workspace-doc-consistency-2026-05",
    ...
    def test_live_reader_rejects_multiple_or_unterminated_ledger_fences(self):
        ...
        for candidate in (valid + valid, valid.removesuffix("```\n")):
            with (
                self.subTest(terminated=candidate.endswith("```\n")),
                mock.patch.object(
                    self.archive,
                    "_read_regular",
                    return_value=candidate.encode(),
                ),
                mock.patch.object(
                    self.archive,
                    "_approved_migration_document",
    ...
    def test_compact_live_reader_rejects_missing_extra_duplicate_unsafe_and_wrong_recovery(
        self,
    ):
        cases = []
        ...
        cases.append(candidate)
        candidate = self.compact()
        candidate["rows"][-1]["recovery_commit"] = (
            "889d3868ecd0913cddac79a718584a54a8453525"
        )
        cases.append(candidate)
    ...
    def test_execution_changes_are_limited_to_completed_status_and_proved_recovery(
        self,
    ):
        for key, value in (
            ("status", "unknown"),
...
class ArchiveMinimizationTests(unittest.TestCase):
    ...
    def test_recovery_validator_rejects_unsafe_values_and_non_blobs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run(
                ["git", "config", "user.name", "Archive Test"], cwd=root, check=True
            )
            ...
            subprocess.run(
                ["git", "add", "body.md", "tree/item.md", "link.md"],
                cwd=root,
                check=True,
            )
            ...
            codes = {
                item.code for item in self.archive.validate_recovery_rows(rows, root)
            }
        ...
        with mock.patch.object(git_provenance, "_run_git") as run_git:
            for commit, path in invalid:
                with self.subTest(commit=commit, path=path):
                    proof = git_provenance.verify_recovery_blob(
                        path, commit, repo_root=ROOT
                    )
                    self.assertFalse(proof.exists)
                    self.assertFalse(proof.is_regular_blob)
    ...
    def test_recovery_row_shape_rejects_null_line_sha_and_unknown_fields(self) -> None:
        ...
        self.assertEqual(
            self.archive.RecoveryReference(
                TASK10_BASELINE, pathlib.PurePosixPath("docs/retired.md")
            ),
        ...
        with self.assertRaisesRegex(ValueError, "minimal contract"):
            self.archive._parse_tombstone_text(
                ...
                pathlib.PurePosixPath(
                    "docs/98.archive/tombstones/03.specs/0001-sample.md"
                ),
                "0001-sample.md",
            )
